# EPS Revision Momentum Strategy v7.2.2 - 상세 기술 문서
...
4. [핵심 알고리즘](#4-핵심-알고리즘)
5. [v7.2 메시지 간소화](#5-v72-메시지-간소화)
6. [v7.1 밸류+가격 100점 체계](#6-v71-밸류가격-100점-체계)
7. [v7.1 텔레그램 자동화](#7-v71-텔레그램-자동화)
8. [v7.0 신규 기능](#8-v70-신규-기능)
9. [데이터 흐름](#9-데이터-흐름)
10. [모듈별 상세](#10-모듈별-상세)
11. [설정 가이드](#11-설정-가이드)
12. [설치 및 실행](#12-설치-및-실행)
13. [백테스팅 설계](#13-백테스팅-설계)
14. [트러블슈팅](#14-트러블슈팅)
...
### 5.1 Option A - 매수 가능 종목만 표시

v7.2.2에서는 **순위 = 매수 우선순위** 원칙을 명확히 하기 위해 매수 가능 종목만 표시:

```python
def is_actionable(row):
    """매수 가능 여부: 액션이 매수 관련 + RSI < 70"""
    action = str(row.get('action', ''))
    rsi = row.get('rsi', 50)
    is_buy_action = any(x in action for x in ['강력매수', '적극매수', '매수적기', '저점매수'])
    is_not_overheated = rsi < 70
    return is_buy_action and is_not_overheated
```
