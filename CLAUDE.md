# 🇺🇸 US 전략 — eps-momentum-us (v119 제3방안 fwd_PE<15 저평가 보유, 메인 cron 활성, 2026-06-12)
...
> **★ PER 보유기준(PE_HOLD) 완화 후보 — 사용자 결정 대기 (2026-06-14)**: 현재 top20 중 **18/20이 PER 15+**(NVDA19.9·AVGO23.3·AMZN26·LLY28 등 고PER 성장 리더십). v119 PE<15 carryover는 SNDK/MU(PER9~11) 시클리컬에 맞춰져 **고PER 메가엔 거의 미발동**(순위 밀리면 즉시매도). 스윕(research/opt_exwinner.py, SNDK/MU제외 robust): **PE15=PE20 동일(완화 무비용)**, **PE40 +50.2%(+10.7%p)**, 순위만 +48.5%, **MDD는 PE무관 −19% 동일**(거품방어 근거 약함). 트레이드오프: 메가포함 full에선 PE40 −12%p(미래 winner가 싼시클릭이면 PE15 유리). **권고=PE_HOLD 15→25~30 완화(고PER 성장시대) or PE20 무비용**. daily_runner.py PE_HOLD 상수 한 줄. 미적용(사용자 결정 후). 봇 send_per_check 발송.
