...
# ★★2026-07-31 LOOK-AHEAD 경고 — 아래 캘리브레이션 수치(Cal 1.49->2.01 등)는 신뢰 불가.
#   당시 검증은 '신호를 당일 종가로 판정해 당일 수익에 적용'해 미래를 훔쳐봤다.
#   정직하게(판정 다음날 체결) 재측정한 결과 — 메모리 바스켓 10년:
#     경보 무시      CAGR +50.3% / MDD -43.8% / Calmar 1.15
#     경보 적용(정직) CAGR +50.1% / MDD -38.9% / Calmar 1.29
#   => 수익 이득 0, MDD만 ~5%p 개선. 구 보고치(CAGR +46.4->+53.1, Cal 1.05->1.61)는 허수.
#   ★'즉시 발동/즉시 해제가 최적'이라는 결론도 그 자체가 오염의 증거였다 — 지연이 짧을수록
#     훔쳐보는 양이 늘어 성적이 좋아진 것. 확인일수가 긴 신호(국면 오버레이 15일)는 무오염 확인됨.
#   ★해제일수(N_CLEAR) 스윕도 재측정 필요: lag0에선 1일 Cal 2.42 vs 5일 1.73으로 큰 차이였으나
#     lag1에선 1.29 vs 1.22로 거의 무차 = 2026-07-09에 5일을 고른 것도, 오늘 바꾸려던 것도 무의미.
#   현재 이 경보는 '주의 표시' 전용(매매 개입 0)이라 실피해 없음. 매매 적용은 재검증 전 금지.
#   research/_lookahead_audit_2026_07_31.py
# ★2026-07-31 K_FIRE 3→4 시도 후 원복(3 유지). 기록으로 남기는 이유 = 같은 실수 반복 방지.
#   경위: 경보를 매매명령→주의표시로 강등한 뒤 "임계도 올려야 하지 않나"(4/6)를 검토.
#   ①예측 확률 기준으로는 4/6이 우세했음 — 발동 후 낙폭 발생확률/무조건부 기준선(리프트),
#     H(30/60/120)×THR(−10/−15/−20) 9칸: 3/6 평균 1.11(4칸이 1.0 미만) vs 4/6 평균 1.39(0칸).
#   ②★그러나 그건 잘못된 목적함수였다. 리프트는 낙폭의 '빈도'만 재고 '크기'를 안 본다.
#     손실 회피(경보 따랐을 때 실제 성과) 기준 10년 실측 — 메모리 바스켓 대비:
#       무시 CAGR +46.4%/MDD −44.0%/Cal 1.05
#       3/6  +53.1% / −32.9% / 1.61  ← 최고 (CAGR·Calmar 모두)
#       4/6  +49.8% / −33.8% / 1.47
#       5/6  +51.5% / −43.1% / 1.19
#     3/6은 헛울림이 많지만 −38%(2022)급 대형 급락을 더 잘 잡는다(재현율 81% vs 73%).
#     대형 급락 1회를 놓치는 손실 > 헛방어 20회의 비용.
#   ③게다가 '주의 표시'로 강등된 뒤엔 헛방어 비용이 사실상 0(매매를 안 하므로) →
#     놓치지 않는 것(재현율)이 유일하게 중요해져 역시 3/6.
#   교훈: 파라미터는 '역할'뿐 아니라 '무엇을 최소화할지'까지 정하고 골라야 한다.
#   research: _memalert_precision_2026_07_31.py · _memalert_variants_2026_07_31.py
...
def build_message():
    ...
    if fired:
        ...
        #   10년 실측(research/_memalert_precision_2026_07_31.py): 발동 29회 중 실제 −20% 급락
        #   5회 = 정밀도 17%. 재현율은 71%(진짜 위기 3번은 모두 사전 포착)라 '놓치진 않지만
        #   자주 헛울리는' 신호 → 명령으로 쓰면 10년간 29회 왕복매매. 게다가 시스템 추천 종목과
        #   충돌(SNDK 3위인데 "메모리 전량 매도")했음. 정보 가치는 유지:
        #   ON 구간 메모리 바스켓 연율 −4.3% vs OFF +81.4%.
        lines = ['🚦 <b>메모리 위험 감시등: 🟠 주의</b>', '',
                 f'메모리 대표 {len(CLUSTER)}종목 중 <b>{n}개</b>가 하락',
                 '추세로 꺾였습니다.',
                 ...
                 '즉 "놓치진 않지만 자주 헛울리는"',
                 '신호라, 팔라는 뜻이 아니라',
                 '<b>주의하라는 뜻</b>으로 보시면 됩니다.', '',
                 '매매는 평소대로 TOP5 신호를 따르시고,',
                 '메모리 종목은 변동성이 평소보다',
                 '클 수 있다는 점만 감안하세요.', '',
                 f'참고 · {exp_line}']
    else:
