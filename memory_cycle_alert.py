...
# ★2026-07-31 K_FIRE 3→4 (사용자 제안 "4/6이라든지"): 역할이 바뀌면 파라미터도 역할을 따라야 함.
#   구 3/6은 2026-07-09에 '메모리 타이밍 매매의 Calmar'로 고른 값이었고, 이 경보는 같은 날
#   매매 명령 → 주의 표시로 강등됐다(정밀도 17%). 판정 기준이 'Calmar'에서 '예측력'으로 바뀜.
#   ★예측력 실측(research/_memalert_variants_2026_07_31.py): 발동 시점 기준 향후 낙폭 발생
#   확률 / 무조건부 기준선 = 리프트. H(30/60/120)×THR(−10/−15/−20) 9조합:
#     3/6 → 0.80~2.04, 평균 1.11, 9개 중 4개가 1.0 미만(= 켜져도 낙폭 확률이 안 오름)
#     4/6 → 1.00~1.72, 평균 1.39, 1.0 미만 0개
#     5/6 → 1.09~1.88, 평균 1.38 (4/6과 동급이나 재현율 51%로 급락, 진짜 위기를 놓침)
#   ⚠️구 근거였던 'ON 구간 메모리 연율 −4.3% vs OFF +81.4%'는 예측이 아니라 동시대적 관계
#     (주가가 이미 MA90 아래면 최근 수익이 나쁜 건 동어반복) — 예측력 판정에 쓰면 안 됨.
#   ⚠️한계: 발동 21~29회 소표본 + 60일 창 중첩으로 유효표본 더 작음 + SNDK 이력 2025-02~.
#   미국만 4종(3/4)이 리프트 1.55로 더 높았으나 SNDK 결측기에 분모가 3으로 바뀌는 아티팩트라 미채택.
MA, K_FIRE, N_CONFIRM, N_CLEAR = 90, 4, 1, 5
