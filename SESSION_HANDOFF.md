...
| 지표 | 장점 | 단점 | 판정 |
|------|------|------|------|
| **ICE BofA HY Spread** | 선행성 최고, 신용시장이 먼저 반응 (2007, 2020) | 절대값보다 변화 속도가 중요 | **메인** |
...
```
스프레드 ≤ 3.5%  → 정상: 퀀트 풀 투입
스프레드 4.0~5.0% → 경계: 퀀트 비중 절반 축소
스프레드 ≥ 5.0%  → 위험: 신규 진입 중단, 기존 포지션 축소
스프레드 ≥ 7.0%  → 위기: 현금 극대화, 레버리지 ETF 저가 매수 준비
```

### 실전 고려사항
- **변화 속도**: 3.0→4.0% vs 6.0→4.0%는 의미가 다름. 20일 이동평균 기울기 병행 필요
- **데이터 소스**: FRED API `BAMLH0A0HYM2` (하이일드 스프레드), yfinance `^VIX`
- **연동 방식 후보**:
  - A) 매크로 신호 → 퀀트 전체 비중 조절
  - B) 매크로 신호 → 포트폴리오 종목 수 조절 (5→3→0)
  - C) 매크로 신호 → 메시지에 경고만 표시 (판단은 사람이) ← 가장 현실적 시작점

### 다음 단계
- 구현 시점: 백테스트 데이터 축적 후
- 우선순위: 기존 미결(지수 경고 SPY/QQQ MA) → 현금 비중 매크로까지 확장
