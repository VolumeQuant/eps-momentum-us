...
106. **의견 하향 비율 기반 (v20)**: rev_down ≥ 3 (절대) → rev_down/(up+down) > 0.5 (비율). GM처럼 ↑17↓4인 대형주가 ↓4≥3이라 제외되던 문제 해소.
...
*v20 업데이트: Claude Opus 4.6 | 2026-02-11 회사 PC — Simple & Clear: Death List 제거, Top 30 통일, 투자 가이드 재작성, 메시지 3분리, AI 리스크 필터, 포트폴리오 강화, 애널리스트 max(0y,+1y), 리스크 필터 철학 확립*
