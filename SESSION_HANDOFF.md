...
114. **이탈 종목 사유 표시 (v20)**: 이탈 종목에 사유 분석 추가. 이평선 이탈, 의견 하향(↓/↑), EPS 둔화, 주가 선반영, 순위 밀림 중 해당 사유 표시. 매도 판단에 핵심 정보 제공.

*v20 업데이트: Claude Opus 4.6 | 2026-02-11 회사 PC — Simple & Clear: Death List 제거, Top 30 통일, 투자 가이드 재작성, 메시지 3분리, AI 리스크 필터, 포트폴리오 강화, 애널리스트 max(0y,+1y), 리스크 필터 철학 확립, 과거 데이터 보충, 마켓 날짜 자동 감지, ON CONFLICT 보존, 섹터 분산, 이탈 사유 표시*
