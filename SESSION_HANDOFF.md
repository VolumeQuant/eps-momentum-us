...
> **v32**: 2026-02-19 집 PC — Risk Consistency 8대 개선: 매출10% 필터 복원, rank 버그 수정, 이탈 사유 구분, 하향 비율 이중체크, 섹터 집중 경고, 유지 구간 표시, 어닝 [4/4] 표시, 차등 비중
...
| 6 | 유지 구간 | 30개만 표시 | **31~35위** 동일 UI 표시 + 가이드 "Top 35" → "목록" 통일 |
| 7 | 어닝 [4/4] | [3/4]에서만 경고 | [4/4] 비중 요약 아래에 **📅 어닝 인라인 표시** |
| 8 | 비중 | 균등 20% | **차등 25/25/20/15/15** (composite 상위 우대) |
...
### 핵심 코드 변경
- `get_part2_candidates()`: rev_growth < 0.10 필터 + NaN 제외
- `create_candidates_message()`: rank 버그 수정, 이탈 분류, 유지 구간 동일 UI
- `run_portfolio_recommendation()`: 차등 비중, 섹터 경고, 어닝 인라인
- AI 리스크 필터 + 포트폴리오 필터: 하향 비율 `↓≥↑ & ↓≥2` 추가
- `create_guide_message()`: "Top 35" → "목록" 통일
