...
> **v24**: 2026-02-12 집 PC — 현금 비중 권장: 분면+지속일수 기반 현금/행동 표시, 종목 수 5개 고정(분산 유지)
...
| 분면 | 조건 | 아이콘 | 메시지 표시 |
|------|------|--------|------------|
| Q1 회복 | HY ≥ 중위수 + 3개월 전보다 하락 | 🟢 | `🟢 신용시장 — 회복기 (HY x%, 중위 m%)` |
| Q2 성장 | HY < 중위수 + 3개월 전보다 하락 | 🟢 | `🟢 신용시장 — 성장기 (HY x%, 중위 m%)` |
| Q3 과열 | HY < 중위수 + 3개월 전보다 상승 | 🟡 | `🟡 신용시장 — 과열 주의 (HY x%, 중위 m%)` |
| Q4 침체 | HY ≥ 중위수 + 3개월 전보다 상승 | 🔴 | `🔴 신용시장 — 방어 모드 (HY x%, 중위 m%)` + `⚠️ 신규 매수 시 신중하게 판단하세요.` |
...
| # | 조건 | 표시 | 30년 근거 |
|---|------|------|----------|
| 1 | HY 4~5% 구간 + 일일 ≤-20bp | `💎 스프레드 급축소 (HY x%, 전일 대비 -Nbp)` | 60일 +6.22% |
| 2 | 전일 HY ≥5% → 오늘 <5% | `💎 5% 하향 돌파 — 올클리어` | 60일 +4.17% |
| 3 | 60일 고점 대비 -300bp 이상 | `💎 고점 대비 -Nbp 하락 — 강력 매수 신호` | 60일 +11.89% |
| 4 | 전일 Q4 → 오늘 Q1 | `💎 침체→회복 전환 — 최고 매수 구간` | 250일 +12.69% |
...
🟢 S&P 500  6,068 (+0.47%)
🟡 나스닥  19,643 (-0.16%)
🟢 신용시장 — 성장기 (HY 2.84%, 중위 3.76%)  ← 여기
```

#### 현금 비중 권장 (v24, 30년 EDA 기반)

종목 수는 항상 **5개 유지** (분산 유지). 비중만 조절:
- 현금 40%이면 → 5종목 × 12% = 60% 주식
- 현금 0%이면 → 5종목 × 20% = 100% 주식

| 분면 | 지속 기간 | 현금 비중 | 핵심 행동 | 30년 근거 |
|------|----------|----------|----------|----------|
| Q2 성장 | - | 0% | 정상 운영 | 연율 +9.4%, 양수확률 84% |
| Q3 과열 | <60일 | 0% | 주의 관찰 | 60일 +1.84%, 아직 양수 |
| Q3 과열 | ≥60일 | 20% | 신규 진입 축소 | 60일 +0.39%, 거의 제로 |
| Q4 침체 | 1~20일 | 20% | 신규 매수 중단 | 60일 +3.37% (반등), 급매도 금지 |
| Q4 침체 | 21~60일 | 40% | 단계적 축소 | 60일 +0.54%, 모멘텀 약화 |
| Q4 침체 | 61일+ | 60% | 적극 방어 | 60일 **-2.38%**, 본격 하락 |
| Q1 회복 | - | 0% | 적극 매수 | 연율 +14.3%, 양수확률 86% |

**핵심 통찰**: Q4 초기 ≠ 패닉. 초기 반등(+3.37%) 있으므로 단계적 축소가 정답.

#### 케이스별 출력 예시

**Case 1: Q2 성장기 (평상시)**
```
🟢 신용시장 — 성장기 (HY 2.84%, 중위 3.76%)
📊 투자 100% · 정상 운영
```

**Case 2: Q3 과열 주의 (초기)**
```
🟡 신용시장 — 과열 주의 (HY 3.50%, 중위 3.76%)
📊 투자 100% · 주의 관찰
```

**Case 2b: Q3 과열 60일+ (장기)**
```
🟡 신용시장 — 과열 주의 (HY 3.55%, 중위 3.76%)
📊 현금 20% · 신규 진입 축소 (Q3 72일차)
```

**Case 3a: Q4 방어 모드 (초기 1~20일)**
```
🔴 신용시장 — 방어 모드 (HY 4.80%, 중위 3.80%)
📊 현금 20% · 신규 매수 중단 (Q4 12일차)
⚠️ 신규 매수 시 신중하게 판단하세요.
```

**Case 3b: Q4 방어 모드 (중기 21~60일)**
```
🔴 신용시장 — 방어 모드 (HY 5.20%, 중위 3.80%)
📊 현금 40% · 단계적 축소 (Q4 35일차)
⚠️ 신규 매수 시 신중하게 판단하세요.
```

**Case 3c: Q4 방어 모드 (장기 61일+)**
```
🔴 신용시장 — 방어 모드 (HY 5.80%, 중위 3.80%)
📊 현금 60% · 적극 방어 (Q4 85일차)
⚠️ 신규 매수 시 신중하게 판단하세요.
```
...
**Case 4: Q1 회복기 + 급축소**
```
🟢 신용시장 — 회복기 (HY 4.50%, 중위 3.80%)
📊 투자 100% · 적극 매수
💎 스프레드 급축소 (HY 4.50%, 전일 대비 -25bp)
```

**Case 5: 올클리어 (5% 하향 돌파)**
```
🟢 신용시장 — 회복기 (HY 4.95%, 중위 3.80%)
📊 투자 100% · 적극 매수
💎 5% 하향 돌파 — 올클리어
```
...
```
🟢 신용시장 — 회복기 (HY 6.80%, 중위 3.80%)
📊 투자 100% · 적극 매수
💎 고점 대비 -350bp 하락 — 강력 매수 신호
```

**Case 7: 최고 매수 구간 (Q4→Q1 + 복합 신호)**
```
🟢 신용시장 — 회복기 (HY 4.60%, 중위 3.80%)
📊 투자 100% · 적극 매수
💎 스프레드 급축소 (HY 4.60%, 전일 대비 -30bp)
💎 침체→회복 전환 — 최고 매수 구간
```
