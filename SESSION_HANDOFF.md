...
112. **포트폴리오 섹터 분산 시도 후 원복 (v20)**: 섹터당 1종목 + 순위 기반 고정 비중(25/25/20/15/15%) 시도했으나 원복. 기존 adj_gap 상위 5개 + adj_gap 비례 비중 유지.
113. **워크플로우 git merge 방식 변경 (v20)**: `git pull --rebase` → `git pull --no-rebase -X ours`. binary DB 파일은 rebase 불가하여 충돌 발생 방지.

*v20 업데이트: Claude Opus 4.6 | 2026-02-11 회사 PC — Simple & Clear: Death List 제거, Top 30 통일, 투자 가이드 재작성, 메시지 3분리, AI 리스크 필터, 포트폴리오 강화, 애널리스트 max(0y,+1y), 리스크 필터 철학 확립, 과거 데이터 보충, 마켓 날짜 자동 감지, ON CONFLICT 보존, 섹터 분산 시도→원복*
