...
- Watchlist: "괴리 1등(-33%) · 매출 3등(+61%)" (등수+실제값)
...
4. **주도 업종**: 헤더 → 하단 범례 위로 이동 (한국 프로젝트와 동일)
5. **선정과정**: `▸` 불릿, "3명+" → "3명 이상"
6. **종목 리스트**: 번호+볼드 한 줄씩
7. **팩터등수**: raw값(EPS% 매출% 괴리%) → 등수 기반 (cherry-pick 방지)
8. **범례 업데이트**: 괴리·매출 등수 설명, EPS추이 4구간 설명

### 파일 변경
- `daily_runner.py`: compute_factor_ranks() 신규, create_v2_signal/watchlist 팩터등수 적용, AI 프롬프트 확장
- `quick_test_v2.py`: compute_factor_ranks import + 전달
