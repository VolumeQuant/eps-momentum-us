...
> **v43**: 2026-02-26 집 PC — Dead Code 제거: 빈 스텁 9개 + 일회성 스크립트 5개 + EDA 4개 삭제, daily_runner.py v1 전용 함수 5개(~896줄) 제거, message_version 기본값 "v3"
> **v43.1**: 2026-02-26 집 PC — 고객 친화 멘트: Q3 톤다운(급전환→주의/신중), portfolio_mode 항상 normal(TOP 5), US/KR 14개 멘트 완전 동일화

---

## v43.1 — 고객 친화 멘트 + 항상 TOP 5 (2026-02-26)

### 배경
- VIX 19.6 상황에서 Q3 멘트 "매수 축소, 급전환에 대비하세요"가 과격
- Q3은 HY < 10년 중앙값이고 3개월 대비 상승 — 역사적으로 매우 낮은 수준에서 "급전환" 불균형
- 고객 관점: Q1/Q2는 적극적, Q3는 신중, Q4는 강하게 — 단계적 그라데이션

### 원칙
- **Q1/Q2**: 기회일 때 적극적 — 변경 없음
- **Q3**: 과격→신중 톤다운 — "급전환"/"매도 검토"/"매수 보류" 삭제
- **Q4**: 심각할 때 강하게 — 변경 없음
- **항상 TOP 5**: portfolio_mode 항상 'normal' — 시장 경고는 멘트로만 전달
...
| 파일 | 변경 |
|------|------|
| `daily_runner.py` | Q3 4개 멘트 톤다운, `portfolio_mode = 'normal'` 고정 |

---

## v43 — Dead Code 제거 (2026-02-26)

### 삭제 파일 (18개, ~1,910줄)

빈 스텁 9개: `_bootstrap.py`, `_writer.py`, `_gen.py`, `write_backtest.py`, `write_bt.py`, `gen_bt.py`, `bt_builder.py`, `bt_main.py`, `backtest_top5.py`

일회성 스크립트 5개: `migrate_weighted_ranks.py`, `recalc_ranks.py`, `recalc_margin_filter.py`, `backfill_ma120.py`, `fix_0217_ranks.py`

EDA 분석 4개: `hy_analysis.py`, `hy_analysis2.py`, `hy_analysis3.py`, `hy_analysis4.py`

### daily_runner.py v1 코드 제거 (~896줄)
- `create_guide_message()`, `create_market_message()`, `create_candidates_message()`, `run_ai_analysis()`, `run_portfolio_recommendation()` 5개 함수 삭제
- v1 디스패치 블록(else 분기) + 사전 계산 변수 제거
- 기본 config `message_version` → `"v3"`
...
> ⚠️ **v43.1에서 Q3 전면 교체**, Q2+VIX warn 변경. 최신 버전은 v43.1 섹션 참조.
