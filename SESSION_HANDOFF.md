...
> **v34**: 2026-02-19 집 PC — 날씨 아이콘 설명 추가: 📖투자 가이드에 종목별 추세 읽는 법 섹션 + [2/4] 간략 범례. 고객이 🔥☀️🌤️☁️🌧️ 아이콘 의미를 바로 이해할 수 있도록 개선

---

## v34 — 날씨 아이콘 설명 추가 (2026-02-19)

### 배경
- 고객 입장에서 종목별 날씨 아이콘 4개(예: `☀️☀️🔥☀️`)가 설명 없이 표시되는 문제
- 📖 투자 가이드에는 시장 계절 아이콘(🌸☀️🍂❄️)만 설명, 종목 추세 아이콘은 미설명
- 임계값 통계 검증 완료: Top 30 분포 🔥5.7% ☀️28.9% 🌤️45.6% ☁️18.8% 🌧️1.0% → 현재 임계값 적절

### 변경 사항
1. **📖 투자 가이드** (`create_guide_message()`)에 `📈 종목별 추세 읽는 법` 섹션 추가
   - 아이콘 4개 = 90→60→30→7→오늘 구간별 EPS 변화
   - 왼쪽이 과거, 오른쪽이 최근
   - 5개 아이콘 범례 + 가속/둔화 예시 2개
2. **[2/4] 매수 후보** 읽는 법에 간략 범례 1줄 추가
   - `추세 아이콘 (90→60→30→7→오늘):`
   - `🔥폭등 ☀️강세 🌤️상승 ☁️보합 🌧️하락`

### 임계값 (변경 없음)
- 🔥 >20% · ☀️ 5~20% · 🌤️ 1~5% · ☁️ ±1% · 🌧️ <-1%
- 4구간: seg1(7d→오늘), seg2(30→7d), seg3(60→30d), seg4(90→60d)
