...
> **v32**: 2026-02-19 집 PC — Risk Consistency + UI 정리: 매출10% 필터 복원, rank 버그 수정, 이탈 사유 구분, 하향 비율 이중체크, 섹터 집중 경고, 어닝 [4/4] 표시, 차등 비중, Top30 통일(버퍼존 제거), UI 가독성 개선
...
- `create_candidates_message()`: rank 버그 수정, 이탈 📉 헤더+분류, 버퍼존 UI 제거
- `run_portfolio_recommendation()`: 차등 비중, ⚠️ 주의사항 통합(시장/어닝/섹터)
