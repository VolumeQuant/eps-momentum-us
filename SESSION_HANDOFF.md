...
> **v46**: 2026-03-06 집 PC — 상관관계 기반 분산 선정: Top 5 선정 시 90일 주가상관 ≥0.65 그룹당 최대 2종목, Signal/Watchlist 의견·섹터경고·분할매수 가이드, DB 쿼리 중복 제거, 이탈 사유 오분류 수정
...
### 변경 1: 상관관계 기반 분산 선정 (`_select_with_corr_cap()`)
- Top 5 선정 시 `safe` 전체(✅ 검증 종목)의 90일 상관행렬 계산
- **Union-Find 그룹**: 상관 ≥0.65 페어를 연결 → 연결 그룹당 최대 2종목
- 초기 버전은 직접 페어 카운트 방식 → STX-MU(0.647<0.65) 미달로 MU 통과 버그 → Union-Find로 수정
- SNDK-MU(0.702) + SNDK-STX(0.699)로 세 종목이 하나의 그룹 → 그룹당 2캡 정상 작동
- 상관행렬 계산 실패 시 기존 방식(단순 Top N) 폴백
- **결과**: SNDK(1), STX(2), ~~MU(3)~~, LLY(4), FIX(5), LITE(6) → MU 대신 LITE 선정

### 실제 상관계수 (2026-03-05 기준, 90일)
| 페어 | 상관 | 판정 |
|------|------|------|
| SNDK-MU | 0.702 | ≥0.65 HIGH |
| SNDK-STX | 0.699 | ≥0.65 HIGH |
| STX-MU | 0.647 | <0.65 통과 |
| MU-LITE | 0.492 | <0.65 통과 |
| LLY-모두 | <0.08 | 완전 독립 |
...
### 파일 변경
- `daily_runner.py`: `_select_with_corr_cap()` 신규, `select_portfolio_stocks()` 수정, Signal/Watchlist 메시지 개선, `_get_recent_dates()` 헬퍼, 이탈 사유 버그 수정
