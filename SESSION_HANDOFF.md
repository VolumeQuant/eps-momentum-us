...
| 분면 | 아이콘 | 메시지 | 행동 |
|------|--------|--------|------|
| Q1 회복 | 🟢 | "회복기 — 적극 매수" | 정상 운영 |
| Q2 성장 | 🟢 | "성장기 — 정상 운영" | 정상 운영 |
| Q3 과열 | 🟡 | "과열 주의" | 경고 표시 |
| Q4 침체 | 🔴 | "방어 모드" | 매수 억제 |

- FRED API로 HY spread 수집 (키 불필요, CSV 직접 다운로드)
- 10년 롤링 중위수 계산 → DB 저장 또는 매일 계산
- [1/3] 메시지 상단에 분면 + HY 수준 표시
...
### 다음 단계
- Method C 코드 구현: FRED 수집 → 분면 판정 → 메시지 표시
- 추가 분석 후보 중 스프레드 변동성, VIX 괴리 검토
