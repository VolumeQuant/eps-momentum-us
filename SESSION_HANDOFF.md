...
>
> **v83 전수 맹점 검증 (2026-05-24 적용 직후)**: 5 카테고리 점검 모두 통과 (`research/verify_v83_comprehensive.py`). **[1] 시스템 수익률**: sys_cum +208.36% / SPY +9.21% / alpha +199.15%p / 67일 / 6승 0패 — 합리적 범위 (sys > spy, 100~500%). **[2] DB 재계산 spot check**: 6개 무작위 날짜 (02/18, 03/05, 03/25, 03/30, 04/06, 05/22) 직접 재계산 = DB 저장값 100% 일치. **[3] C2 boost edge case**: 30거래일 전 가격 없는 종목 0건 (모두 is_c2=False), eps_chg_weighted ≤ 0 종목 0건 (시스템이 EPS 상향만 진입), 5/22 Top 30 중 C2 6건 (20%) = 정상 boost 작동, is_cold_start() False. **[4] _w_gap (시스템 성과) rerank 정렬 일관성**: _build_score_100_map Top 10 = DB part2_rank Top 10 완전 일치 (3곳 rerank 정합 확인). **[5] 메시지 잔여물 grep**: 70%/70/30/weights_v82/v82_weights/"1위 70%"/"2위 30%" docstring·주석 외 잔여 0건. "매수: 상위 2종목 (1위 80%, 2위 20%)" 2곳 모두 변경 완료. **5/22 메시지 미리보기 (실제 cron 결과)**: 1. AEIS (80%, C2 가격 -14.43% + EPS +12.13), 2. TER (20%, C2 가격 -2.60% + EPS +11.79) — **1위 + 2위 둘 다 C2** = 시스템 본질 (buy-the-dip mean reversion) 정확 작동. **회사 PC 인계 안내**: `git pull` 받으면 즉시 이어 작업 가능. DB (v83 마이그레이션 완료) + daily_runner.py (v83) + backup (`bak_pre_v83`) + 검증 스크립트 모두 commit + push 완료. **롤백 명령**: `cp eps_momentum_data.db.bak_pre_v83 eps_momentum_data.db && git checkout 54ee685 -- daily_runner.py`.
>
> **v83.2 — C2 boost 완전 제거 (2026-05-27)**: **계기**: 사용자가 5/26 메시지에서 BWXT(#2 매수, 20%) 이상 감지 → 분석 결과 BWXT는 C2(EPS+가격 30d -12.2%)라 binary +3 boost로 raw #4 → #2 점프. eps_chg_weighted 2.82로 상위 클러스터 중 최약인데 MU(70)와 동일 +3 = 부당. **검증 (leave-one-superwinner-out, raw w_gap 베이스라인으로 이중boost 차단)**: ① binary vs no_boost 80/20 — binary가 **no_boost보다 -8.64%p 열세**(R lift), M24 min -4.16%p. 4/2 가격추적으로 하베스 검증 (SNDK가 MU보다 일찍 더 오름 → no_boost의 SNDK-80%가 정당히 우위). ② gate(eps≥10) vs no_boost — 전체우주 +2.14%p(486/500)지만 **MU 제외 시 동전던지기(239/500), M24 -0.61%p, 최악 -79%p** = edge 전부 MU 1종목. ③ C1 boost(과거 미적용)도 **SNDK 제외 시 -4.68%p(186/500)** = 동일 착시. → **두 boost 다 single-stock 아티팩트 확정.** 원래 v83 BT(+48%p)는 v82(70/30,3슬롯)→v83(80/20,2슬롯,+boost) 변경을 묶어 측정해 boost 단독 기여 과대평가. **변경**: `daily_runner.py` `_apply_c2_boost_rerank`/`_is_c2_for_v83` 헬퍼 + 상수 + 호출 3곳(save_part2_ranks/_build_score_100_map/_w_gap) 전부 제거 → part2_rank = 순수 w_gap. **DB 마이그레이션** (`research/apply_no_boost.py`): 71일 part2_rank 재계산(42일 변경, 510건), 읽기전용 검증 불일치 0건. cr/score는 boost 무관(불변). **부수 해소**: BWXT가 FIX보다 높던 점수 왜곡 + 궤적(cr) vs 픽(p2) 불일치. **config 재검증 (전부 유지)**: exit10=plateau중앙(최선 exit이 우주별 6/10/12로 튀어 overfit), slots2=위험/수익중앙, 80/20=70/30과 MDD 동일(50/50은 더 나쁨). **깨진 건 boost뿐, (2,10,2)+80/20 합리적.** backup `eps_momentum_data.db.bak_pre_c2gate_20260527`. research: `bt_c2_eps_aware_grid.py`, `bt_c2_gate_vs_noboost_robust.py`, `bt_c1_and_weights_robust.py`, `bt_weights_decision.py`, `bt_exit_slots_decision.py`, `apply_no_boost.py`.
>
> **v83 적용 (2026-05-24, v82 정정)**: 슬롯 (2,10,2) 80/20 + **C2 boost rank +3** production 적용. **계기**: v82 (70/30 b=0) 적용 직후 사용자 우려 "1위 신호 약할 때 80% 부담"으로 dynamic weight 검토 → 80/20 + C1 boost=5 측정 (R lift +40%p) → **24 multistart M24 min -37.50%p tail risk 발견 (4/2 시작일)**. 원인 분석: C1 boost가 MU (C2, 가격 30d -22% + EPS+70%, +103% 슈퍼위너) 4위로 밀어내고 FIVE 80% 진입 → -2%p 손실. **본질 재정립**: 시스템 진입 본질 = adj_gap (괴리율) 기반 mean reversion (buy-the-dip) → **C2가 본질 강화**. 사용자 첫 거부 "C2 boost 추세 추종 모순" 재검토 결과 정반대 — C1 boost가 본질 거스름. **BT 검증** (research/bt_third_way.py, bt_c2_dense_grid.py): 80/20 + C2 rank-boost b=3 = random 500 +48.14%p / 500/500 wins, 24 multistart 24/24 wins, M24 lift +52.76%p, **M24 min +13.54%p**, M24 max +105.97%p, MDD -19.82% (v82 동일). **모든 측정 지표 v82 ≤ v83**, 한 지표도 v82가 우월한 게 없음. **4/2 worst 재검증**: v82 (+13%p) → v83 (+38%p) 완벽 뒤집기 (MU 1위 → 80% 비중 자동). **Dynamic Weight 검토**: Dynamic A (2단계 ratio≥1.2 → 80/20 else 70/30) = M24 lift +51.53%p / M24 min +16.91%p (Fixed 80/20보다 worst +3.37%p ↑, 평균 -1.23%p ↓). 사용자 결정: Fixed 80/20 (평균 알파 최대화 + 단순). **변경**: `_apply_c2_boost_rerank` 헬퍼 신규 (rank score (n-idx) + boost by C2). 3곳 적용 — `save_part2_ranks` 정렬, `_build_score_100_map` w_score_map (분포 보존, 순서 rerank), `_w_gap` in `_get_system_performance` 동일. weights [70,30] → [80,20]. 메시지 "1위 80%, 2위 20%" 2곳. **z-score vs rank 단위 검증** (research/bt_c2_zscore_boost.py): z-score boost b=15가 rank b=3에 가장 가까움 (M24 +48.14% 동일, R lift -5%p 약함) → rank reorder 채택 (BT 정확 매칭). **마이그레이션** (research/apply_v83.py): 70일 part2_rank 재계산, 41/70일 변경, 총 496건. 4/2 part2_rank (1, SNDK)+(2, MU) → **(1, MU)+(2, SNDK)** (MU C2 boost로 1위). **5/22 적용 직전 영향**: Top 10 1위 AEIS 유지 (C2, 가격 -14.43% + EPS +12.13%), **2위 FIX → TER** (TER C2 boost), 메시지 AEIS 80%/FIX 30% → AEIS 80%/**TER** 20%. backup: `eps_momentum_data.db.bak_pre_v83`. **모니터링**: 5거래일 SPY 알파 -5%p 이하 / MDD -10% 초과 / Top 3 교체율 50%+ 시 롤백 검토. **research**: `bt_third_way.py`, `bt_c2_dense_grid.py`, `bt_dynamic_c2.py`, `bt_c2_zscore_boost.py`, `analyze_worst_case_b.py`, `apply_v83.py`, `verify_v83.py`.
>
> **v82 적용 (2026-05-24, v83에 의해 정정됨)**: 슬롯 3→2 + 비중 균등→70/30 (1위 70%, 2위 30%) production 적용. **사용자 토의 흐름이 핵심**: (1) 5/23 BT는 (3,10,2) 80/20 + C2 boost=3 +48%p 추천했으나, (2) 사용자가 C2 boost 추세 추종 본질 모순 지적 + 80/20 부담 + BT 신뢰 의심 → (3) C2 boost 빼고 70/30 b=0 (+27%p) 선택, (4) 추가로 C1 boost=5 검증 (사용자 직관: C1이 어려움 뚫고 상위 = 진짜 슈퍼위너) 7/7 만족 기준 통과 (+39%p) but v81 학습 살려 단계적 보류. **변경**: `daily_runner.py:select_display_top5` MAX_SLOTS 3→2 + weights [70,30], `_get_system_performance` slots/entry/weights 동기화, 메시지 "상위 2종목 (1위 70%, 2위 30%), 최대 2종목 보유". **BT 검증** (`research/bt_corrected_final.py` + `bt_no_boost_options.py`): random 500 +27.43%p / 500/500 wins, 12 multistart +30.89%p / 12/12 wins / M min +12.03%p (음수 0건), MDD -19.98% (baseline -18.14% 대비 +1.84%p 미세 악화), 거래비용 1% stress +27.86%p 통과, 약세장 4/5 sub-period 우월, SNDK 등 C1 슈퍼위너 그대로 잡음 (1, 2위 진입). **BT 버그 발견**: 이전 측정 (bt_combined_grid 등) 슬롯 idx 기반 weight (점수 무관) → 점수 2위가 슬롯 0 점유 시 80% 받음. 정확 BT (점수 기반 weight 재배정)는 +30%p 적은 측정. (3,10,2) 80/20 b=3 = +48.62%p → 정확 +41.17%p. **production v80.2 entry cap 제거 인식**: (2,10,2)와 (3,10,2) 동일 동작 (슬롯 채울 때까지 슬라이드). 표기 (2,10,2)가 정확. **거부된 옵션**: dynamic_5 b=3 (+45%p, 메시지 복잡 + 임계값 over-fit), 80/20 b=3 (+41%p, C2 boost 모순), 80/20 b=0 (+33%p, 1위 비중 부담), dynamic_5 b=0 (+36%p, over-fit 약간), A2 (3,10,3) 50/30/20 (+13%p, 알파 너무 작음). **C1 boost=5 검증 통과** (단 적용 보류): random 500 +39.01%p / 500/500, 12 multistart +42.97%p / 12/12 / M min +15.26%p, MDD -19.98%, simulator 정확 (7/7 trade DB 일치), 약세 4/5 우월, 거래비용 1% +39.47%p 통과, boost 3/4/5/6 sensitivity 일관, **SNDK 17회 진입** (사용자 직관 부합). 사용자 결정: 70/30 b=0 먼저 적용 + 5-10거래일 모니터링 후 C1 boost 검토. **DB 마이그레이션 없음**, backup `eps_momentum_data.db.bak_pre_c1boost_2026_05_24`. **모니터링**: 5거래일 SPY 알파 ≥ +10%p / MDD ≤ -25%, 10거래일 ≥ +15%p. 미충족 시 baseline 복귀. **research**: `bt_corrected_final.py`, `bt_no_boost_options.py`, `bt_c1_boost.py`, `verify_c1_boost_final.py`, `bt_score_ratio_weighting.py`, `bt_2vs3_production.py`, `bt_entry_unlimited.py`, `sim_message_2026_05_22.py`, `analyze_sndk_contribution.py`.

> **Portfolio Optimization Research (2026-05-23)**: 7시간 자유시간 BT 연구. 22개 BT 변형 × 5 Phase. **최강 후보: (3,10,2) 80/20 + C2 boost=3** = random +48.62%p / 500/500 wins / 12 multistart +56.63%p / 12/12 wins / M min +18.43%p (음수 0건) / MDD -19.82% (baseline -18.18% 대비 +1.71%p 미세 악화) / 누적 +241.34% (baseline +147.79%, +93.55%p). **구성**: slots 2 (3→2), weights [80, 20], entry 3, exit 10, C2 boost=3 (EPS↑ + price 30d < 0 → rank score +3). **메커니즘**: MU 결정적 사례 — baseline 4/1 $367 진입 33% 비중 → C2 boost로 3/30 $321 진입 (2일 빨리) + 80% 비중 → portfolio 기여 +34%p → +106%p. C2 진입 5건 avg +35.25% > C1 6건 avg +18.77%. **단독 검증**: A2 (50/30/20 슬롯 3) +3.62%p MDD 동일 (안전 옵션), E (2,10,2) 균등 +27.50%p MDD +4%p, baseline + boost=3 단독 -0.49%p (시너지 효과만). **거부된 후보**: DCA down (-18~-59%p, trend-follower 부적합), Pyramid up (-8~-29%p), Time-based DCA (-4~-18%p), Profit taking (-13~-24%p), Trailing stop (-0.5~-2.3%p), slot 1 (MDD -23.9%), entry 확장 (4/5/10 효과 0), strict price threshold (효과 0), C2 boost 단독 옵션 A (메시지/진입 불일치, 사용자 재현 불가), 옵션 B (random +1.40% / multistart -0.11%p 시점 의존). **boost step function**: 0/1/2 효과 0 → 3에서 +20%p jump → 4/5 동일 → 7 붕괴. **비중 sweet spot**: 50/50 (+37%p, MDD -22%), 70/30 (+45%p, MDD -20%), 80/20 (+49%p, MDD -19.82% ★), 90/10 (+52%p, MDD -22%) — 80/20이 alpha + MDD 둘 다 sweet spot (counter-intuitive). **C2 정의 sensitivity**: EPS 정의 영향 0 (시스템이 강한 EPS만 잡음), price lookback 결정적 (7d -29%p 패배, 30d +25%p 최강, 60d +2%p stale). **simulator 검증** (v81 학습): 11/11 trade 가격 DB 일치, C2 분류 정확, 독립 simulator +232.20%로 일치 (시작일 차이 +9%p), 환상 아님 확정. **한계**: 65일 강세장 단일 환경, 약세장 미검증, 11~12건 작은 표본 (MU +103% 단일 케이스 의존성). **권장 적용**: Option 1 (보수적) — Step 1 A2 (50/30/20) 5거래일 통과 후 → Step 2 (3,10,2) 80/20 boost=3. Option 2 (공격적) — 곧장 적용. **모니터링**: 5거래일 SPY 알파 ≥ BT 예측 50% / MDD ≤ -25%, 10거래일 알파 ≥ 70%. **research**: `research/PORTFOLIO_OPTIMIZATION_2026_05_23.md` (종합 보고서) + `bt_combined_grid.py` (Phase 1) + `bt_310_2_boost_grid.py` (Phase 2) + `analyze_310_2_80_20_boost3.py` (Phase 3) + `bt_extreme_variants.py` (Phase 4) + `verify_310_2_simulator.py` (Phase 5) + `bt_c2_definitions_compare.py` (Phase 6) + 거부된 옵션 BT (bt_dca_strategy/bt_pyramid_and_time/bt_profit_taking/bt_trailing_stop 등).

> **Blind Spots Audit (2026-05-20)**: v81 롤백 다음날 production 코드 맹점 전수 점검. **확실한 버그 fix 2건**: (1) `daily_runner.py:4088` X8 → X10 (commit `6d71491`) — v80.10b 업데이트 누락분. `_get_system_performance` 누적 수익률 계산이 X8 (v78 옛값)로 남아 표시 수익률 부정확. 5/19 측정: X8 +117.23% → X10 +125.10% 차이 +7.87%p (v80.10b BT 예측 +7.08%p 일치). 메시지 헤더 정확화. (2) day_ret divisor 동형 버그 (commit `1301577`) — bt_breakout_hold OLD simulator의 pool-exit price masking 패턴이 production `_get_system_performance` line 4060-4066에도 존재 (`w = 1.0/len(portfolio)` → 가격 누락 종목 0 contribution + 보유 수로 분모). fix: `n_valid` 분모 사용. 65일 production 데이터에서 누락 0건이라 영향 +0%p지만 universe rotation / delisting 대비 예방적 fix. **검증 후 변경 안 함 5건**: (1) 3일 검증 K=3 sliding window — K=1/2/3/4/5 비교 BT, K=4 random +1.96%p (◐) but 12 multistart 시점 의존 (Early 6/6 양수, Late 5/5 음수) → K=3 robust. (2) 어닝 2주 차단 — 12 진입 history에서 어닝 사고 0건, FORM 1건만 어닝 13일 전 진입 +33.37% 비트, 시스템 자체 보호 (min_seg<-2, rank>10) 충분. (3) MA60 fallback 강화 — dense grid (ma40~ma200) NEW simulator로 모두 ma120 worse 확인. MA60 strict -0.56%p / 42 wins ~동등하지만 buy-the-dip 분석에서 **MU 4/1 진입 (MA60 -6.6%) → 26일 보유 +103.02%** 발견 → MA60 strict 시 MU 알파 통째로 손실. MA120 sweet spot 확정. (4) MA60 이탈 경고 (display) — 사용자 self-awareness: "경고 보면 의식해서 안 사고 4등 살 거 같다" → behavioral bias로 알파 갉아먹음. 시스템 trust 우선, 별도 경고 추가 안 함. (5) Score 100 표시 압축 — 현재 `score = ws/max × 100` 비율 방식. min-max 정규화 대안 검증 결과 미세 차이 과장 (AEIS 100/SNDK 89/TER 85 → "AEIS 압도적" 잘못된 인상) + NTRS 20위=0 어색 + max/min 매일 변동 불안정. 현재 비율 방식이 정확한 정보 전달. **검증 스크립트**: `research/verify_x8_x10_perf.py`, `bt_consecutive_K.py` (+ multistart), `analyze_buy_dip_history.py`, `analyze_earnings_proximity.py`, `bt_ma_mdd_detail.py`, `bt_ma_dense_NEW.py`. **인사이트**: simulator 버그 패턴 production 코드에도 존재 (1건) → 신규 코드 추가 시 price_full fallback + n_valid 분모 표준 강제. "경고가 알파 갉아먹는다" 원칙 — risk-tolerant 사용자는 시스템 추천 외 별도 경고 X.

> **v81 (MA20 매수 풀) → ROLLED BACK (2026-05-19)**: 5/18 적용한 v81 MA20 매수 풀이 8중 BT의 simulator 버그가 만든 환상 알파 (+27.57%p)임을 다음날 발견 후 즉시 롤백. **계기**: 사용자 v82a 직관 (MA120 매수 + 명시 MA20 매도) BT 작성 중 `price_full` fallback 추가한 NEW simulator로 v81을 측정하니 -7.59%p / 116/500 wins (← OLD simulator의 +27.57%p / 100/0과 완전 정반대). **smoking gun**: `bt_breakout_hold.py` (OLD simulator)의 `load_data_ext`가 `composite_rank IS NOT NULL`로만 로드 → 보유 종목이 풀 이탈하는 날 `today_data.get(tk)` 미스 → day_ret 0 처리 + `len(portfolio)`로 나눔 → **MA20 break-day의 polum_exit price masking으로 폭락이 invisible**. v81 보유일 중 price 누락 = 2511일 (v80.10c는 131일, 19배 차이). 모든 매도 트리거 day가 OLD에서 숨겨졌고 그게 +27.57%p "알파"의 정체. **8중 BT 무효화**: random 100/0, 12시작일 12/12, 충격 회피 가설 반박, A vs B 매도 분리, 6 파라미터 grid × 15000sim, 48조합 grid, SNDK/슈퍼위너 제외 시 lift 증가, 대칭 (entry=slots) 비교 — 모두 같은 simulator 위에서 한 거 = **같은 버그를 8번 본 것**. 일관성은 신뢰 근거 아님. NEW simulator 1회로 8중 검증 전체 invalidate. **롤백**: `bak_pre_v81` DB 복원 (1864 part2_rank entries, 5/15까지) + `daily_runner.py` `git checkout cc994d2~1` (v80.10c 복귀). 시행 1 거래일 (5/18 cron 미실행, 데이터 손실 0). 안전 백업 `bak_v81_applied_2026_05_19` 보관. **MA 풀 변형 NEW 재검증** (`research/bt_ma_filter_extended_NEW.py`): ma20 OLD +27.57%p → NEW -7.54%p, ma50 OLD +21.46%p → NEW -1.60%p, ma100 OLD +3.18%p → NEW -4.26%p — **모든 풀 필터 강화 알파가 환상**. ma120 (current) sweet spot 확정. **v82a 검토** (MA120 풀 + 명시 MA20 매도): NEW에서 +2.43~2.74%p / 60~72% wins / 12 multistart 7/12 = 시점 의존 (Early 음수, Late 양수). 채택 안 함. **commits**: `de07297` (rollback), `e6a5e26` (research). **인사이트**: 풀 필터 강화 BT는 자동 의심 대상 (회전 빈도가 알파처럼 보이는 환상의 본거지). 매도 룰 변경은 안전 (종목이 풀에서 안 빠진 채 매도되므로 simulator 영향 없음). 미래 누가 "더 엄격한 풀 필터 + 일관된 BT 8중 확인" 들고 오면 즉시 NEW simulator 재검증 필수.
