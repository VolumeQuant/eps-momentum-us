...
### 표시/DB 불일치 수정 (v36.2)
- **문제**: `create_candidates_message()`가 `get_part2_candidates(top_n=30)` 호출 → 당일 composite Top 30 표시
  `save_part2_ranks()`는 가중순위 Top 30 DB 저장 → 최대 8종목 불일치
- **증상**: TTMI가 가중순위 Top 30(rank 25)이지만 composite Top 30 밖(rank 31) → 표시에서 사라짐 + Death List에도 안 나옴 (유령 상태)
- **수정**: `create_candidates_message()`에 `today_tickers` 파라미터 추가, DB 저장 목록과 동일한 종목 표시
- **영향**: FTI, FIX 등 composite Top 30이지만 가중순위 밖인 종목은 더 이상 표시 안 됨 (정확한 동작)
