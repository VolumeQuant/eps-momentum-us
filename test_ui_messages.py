...
# 2) [1/3] 매수 후보 (가짜 데이터로 직접 생성)
msg_part2 = create_part2_message(df, status_map, exited_tickers, market_lines, top_n=25)
print(f"[2] [1/3] 매수 후보 ({len(msg_part2)}자)")

# 3) [2/3] AI 리스크 필터
print(f"[3] [2/3] AI 리스크 필터 ({len(fake_ai_msg)}자)")

# 4) [3/3] 최종 추천
print(f"[4] [3/3] 최종 추천 ({len(fake_portfolio_msg)}자)")
...
messages = [
    ('📖 투자 가이드', msg_guide),
    ('[1/3] 매수 후보', msg_part2),
    ('[2/3] AI 리스크 필터', fake_ai_msg),
    ('[3/3] 최종 추천', fake_portfolio_msg),
    ('시스템 로그', msg_log),
]
