# -*- coding: utf-8 -*-
"""충실판 B2: gap을 conviction 배율에 녹임 (rev_bonus 옆에 '기대성장 보너스').
   conviction = max(상향비율,|NTM변화|) + rev_bonus + W*max(0,log(gap))  → conv_gap=adj_gap*(1+conv)"""
...
def conviction(tk,d,W,cap):
    v=D[d][tk]; ag=v['ag'] or 0; up=v['up']; na=v['na']; nc=v['nc']; n90=v['n90']; rg=v['rg']
    ...
    if W!=0:
        g=gap(tk,d)
        if not np.isnan(g): gb=min(W*max(0.0,np.log(g)), cap)  # 기대성장 보너스(>1만 보상)
    return ag*(1+base+rb+gb)
def day_scores(d,W,cap):
    dd=D.get(d,{}); tks=list(dd.keys())
    if not tks: return {}
    conv=np.array([conviction(t,d,W,cap) for t in tks],float)
    m,s=np.mean(conv),np.std(conv)
    return {t:(max(30.0,65+(-(conv[i]-m)/s)*15) if s>0 else 65) for i,t in enumerate(tks)}
def run(W=0.0,cap=0.5,ban=()):
    DS={d:day_scores(d,W,cap) for d in all_dates}
    nr={}
    for k,d in enumerate(all_dates):
        ...
        for t in DS[d]:
            ...
            for i,dd_ in enumerate(win):
                ...
                if dd_!=d and (t not in D.get(dd_,{}) or D[dd_][t].get('p2') is None): sc=MISS
                else: sc=DS.get(dd_,{}).get(t,MISS)
                s+=sc*wts[i]
            wg[t]=s
        order=sorted(wg,key=lambda t:-wg[t]); nr[d]={order[i]:i+1 for i in range(len(order))}
    ...
    for k in range(2,len(all_dates)):
        ...
        for t in list(pf.keys()):
            ...
            r=rk.get(t); nc=v['nc']; m=minseg(v); sell=False
            if m<-2: sell=True
        ...
        if len(pf)<2:
            ...
            cands=[t for t,_ in elig if t not in pf and t in tradable and t not in ban and ms.get(t,-9)>=0 and rk.get(t,999)<=5 and (dd.get(t,{}).get('dv') or 0)>=1000]
...
base=run(0.0)
print(f"SANITY W=0: {base[0]:+.1f}% MDD{base[1]:.1f}% {base[2]}\n")
print("=== B2: conviction에 기대성장 보너스 W*max(0,log gap) (cap0.5) ===")
res={}
...
for w in ['MU','SNDK','STX','NVDA','LITE','AVGO']:
    b=run(0.0,ban=(w,))[0]; r=run(bestW,ban=(w,))[0]; worst=min(worst,r-b)
    print(f"  -{w:5}: Δ{r-b:>+7.1f}p")
