# -*- coding: utf-8 -*-
"""두 시스템 포트폴리오 레벨 결합: 2슬롯 + sleeve 자본배분. 합치면 risk-adjusted 좋아지나?"""
...
def sleeve_rets():
    ...
    for d in all_dates:
        ...
        if m!=rebm or not hold:
            ranked=sorted([(t,gap(t,d)) for t in D.get(d,{}) if gap(t,d) is not None and t in PX.columns and (dvol(t,d) or 0)>=1000],key=lambda x:-x[1])
            new=[t for t,_ in ranked[:7] if px(t,d)]
            if new:hold=new;lastpx={t:px(t,d) for t in new};rebm=m
    return r
def twoslot_rets():
    EXIT_RANK=12;PE_HOLD=30;pf={};r={}
    for k in range(len(all_dates)):
        d=all_dates[k];r[d]=0.0
        if k<2: continue
        pv=all_dates[k-1];dd=D.get(d,{})
        ...
        for t,info in pf.items():
            ...
            if cu and pp and pp>0: dr+=w*(cu-pp)/pp
        r[d]=dr
    ...
    return r
sr=sleeve_rets(); tr=twoslot_rets()
sa=np.array([sr[d] for d in all_dates]); ta=np.array([tr[d] for d in all_dates])
def metrics(rets):
    nav=1.0;peak=1.0;mdd=0.0;ser=[]
    for x in rets:
        nav*=(1+x);peak=max(peak,nav);mdd=min(mdd,nav/peak-1);ser.append(nav)
    vol=np.std(rets)*np.sqrt(252)
    sh=((nav**(252/len(rets))-1))/vol if vol>0 else 0
    return (nav-1)*100,mdd*100,sh
print(f"두 시스템 일별수익 상관: {np.corrcoef(sa,ta)[0,1]:+.2f}\n")
print(f"{'배분 (2슬롯/sleeve)':22} {'수익률':>8} {'MDD':>7} {'Sharpe':>7}")
for w in [1.0,0.8,0.7,0.6,0.5,0.3,0.0]:
    blend=w*ta+(1-w)*sa
    cum,mdd,sh=metrics(blend)
    print(f"{f'{int(w*100)}/{int((1-w)*100)}':22} {cum:>+7.0f}% {mdd:>6.0f}% {sh:>7.2f}")
