"""C1 boost=5 환경에서 비중 sweet spot — 모든 비중 × C1 boost 5"""
import sys
import sqlite3
import random
...
def main():
    print('=' * 110)
    print('C1 boost=5 환경에서 비중 sweet spot')
    print('=' * 110)
    ...
    for label, w in weight_options:
        cfg = (w, 30, 10, 5)  # C1 boost=5
        ...
        cfg_label = f'(2,10,2) {label} + C1 b=5'
        print(f'  {cfg_label:<33} {r_avg:+7.2f}% {r_lift:+8.2f}%p {r_wins:>5}/500 {m_avg:+7.2f}% {m_lift:+8.2f}%p {m_wins:>4}/12 {m_min:+8.2f}%p {worst_mdd:+8.2f}%')
