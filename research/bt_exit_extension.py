"""B: Exit threshold extension BT

현재: rank > 10 매도
검증: rank > X (X = 8, 10, 12, 15, 20, 25, 30) 비교

v80.10b 메모리: 8 → 10 변경 시 random 100 +7.08%p (100/100 wins)
가설: 더 길게 보유 (12, 15, 20)하면 회복 케이스 더 잡을지
...
def simulate(dates_all, data, price_full, exit_top, start_date=None,
             entry=3, slots=3):
    if start_date:
    ...
    for di, today in enumerate(dates):
        ...
        for tk in list(portfolio.keys()):
            ...
            if rank is None or rank > exit_top: exited.append(tk); continue
...
def run(exit_top, dates, data, price_full, seed_starts):
    rets, mdds, seed_avgs = [], [], []
    for chosen in seed_starts:
        sr = []
        for sd in chosen:
            r = simulate(dates, data, price_full, exit_top, start_date=sd)
...
def main():
    print('=' * 100)
    print(f'B: Exit threshold extension BT — rank > X 비교')
    print(f'N_SEEDS={N_SEEDS} × {SAMPLES} = {N_SEEDS*SAMPLES} sim/X')
    print('=' * 100)
    ...
    Xs = [8, 10, 12, 15, 20, 25, 30]
    print()
    print(f'{"X":<5} {"avg":>9} {"med":>9} {"worst MDD":>10} {"sharpe":>7}')
    print('-' * 60)
    results = {}
    for X in Xs:
        res = run(X, dates, data, price_full, seed_starts)
        results[X] = res
        ...
        marker = '★' if X == 10 else '  '
        print(f'{marker} X={X:<3} {avg:+8.2f}% {med:+8.2f}% {worst_mdd:+9.2f}% {sharpe:+6.2f}')

    print()
    print('=' * 100)
    print('paired vs X=10 (current baseline)')
    print('=' * 100)
    print(f'{"X":<5} {"avg lift":>10} {"worst":>10} {"best":>10} {"wins":>10} {"verdict":>15}')
    print('-' * 70)
    base = results[10]['seed_avgs']
    for X in Xs:
        if X == 10: continue
        new_ = results[X]['seed_avgs']
        ...
        print(f'  X={X:<3} {avg_l:+8.2f}%p {worst:+8.2f}%p {best:+8.2f}%p {wins:>5}/{N_SEEDS}  {verdict}')
