"""C1 boost BT — 사용자 직관 (주도주 우대) 검증

가설: 우리 시스템 본질 (adj_gap 음수 우대) → C2 자연 상위.
     C1이 상위에 있다는 건 어려움 뚫고 올라온 진짜 슈퍼위너.
     C1에 보너스 주면 SNDK 같은 momentum 강조.

C2 boost (이전 +8%p) vs C1 boost 비교.

테스트:
  (2,10,2) 70/30 + C1 boost {0, 1, 2, 3, 5}
  (2,10,2) 80/20 + C1 boost 3 (참고)
  (3,10,3) 균등 + C1 boost 3 (베이스 환경)
  A2 (3,10,3) 50/30/20 + C1 boost 3
"""
import sys
import sqlite3
import random
...
def calc_weights(weights_str, scores, slots):
    n = len(scores)
    if n == 0: return [0] * slots
    if n == 1: return [100] + [0] * (slots-1)
    if weights_str == 'equal_3':
        return [33, 33, 34]
    elif weights_str == 'A2':
        return [50, 30, 20]
    elif weights_str == '70_30':
        return [70, 30]
    elif weights_str == '80_20':
        return [80, 20]
    return [50, 50]


def simulate(dates_all, data, price_full, weights_str, slots, entry, exit_, c1_boost, start_date=None):
    if start_date:
    ...
    for di, today in enumerate(dates):
        ...
        if len(portfolio) < slots:
            ...
            if new_entries:
                ...
                weights = calc_weights(weights_str, scores, slots)
                new_portfolio = {}
...
def run_random(weights_str, slots, entry, exit_, c1_boost, dates, data, price_full, seed_starts):
    rets, mdds, seed_avgs = [], [], []
    for chosen in seed_starts:
        sr = []
        for sd in chosen:
            r = simulate(dates, data, price_full, weights_str, slots, entry, exit_, c1_boost, start_date=sd)
...
def main():
    print('=' * 110)
    print('C1 boost BT — 추세 슈퍼위너 우대 (사용자 직관 검증)')
    print('=' * 110)
    t0 = time.time()
    ...
    configs = [
        # baseline
        ('baseline (3,10,3) 균등 b=0',  'equal_3', 3, 30, 10, 0),
        ('baseline + C1 boost=1',       'equal_3', 3, 30, 10, 1),
        ('baseline + C1 boost=2',       'equal_3', 3, 30, 10, 2),
        ('baseline + C1 boost=3',       'equal_3', 3, 30, 10, 3),
        ('baseline + C1 boost=5',       'equal_3', 3, 30, 10, 5),
        # A2
        ('A2 (3,10,3) 50/30/20',          'A2', 3, 30, 10, 0),
        ('A2 + C1 boost=3',                'A2', 3, 30, 10, 3),
        # 70/30
        ('(2,10,2) 70/30 b=0',          '70_30', 2, 30, 10, 0),
        ('(2,10,2) 70/30 + C1 boost=1', '70_30', 2, 30, 10, 1),
        ('(2,10,2) 70/30 + C1 boost=2', '70_30', 2, 30, 10, 2),
        ('(2,10,2) 70/30 + C1 boost=3', '70_30', 2, 30, 10, 3),
        ('(2,10,2) 70/30 + C1 boost=5', '70_30', 2, 30, 10, 5),
        # 80/20
        ('(2,10,2) 80/20 + C1 boost=3', '80_20', 2, 30, 10, 3),
    ]

    print()
    print(f'{"config":<37} {"R avg":>9} {"M avg":>9} {"M min":>9} {"worst MDD":>10} {"vs baseline":>15}')
    print('-' * 100)
    results = {}
    for label, w, slots, entry, exit_, b in configs:
        t1 = time.time()
        r_random = run_random(w, slots, entry, exit_, b, dates, data, price_full, seed_starts)
        multi_rets = []
        for sd in fixed_starts:
            r = simulate(dates, data, price_full, w, slots, entry, exit_, b, start_date=sd)
            multi_rets.append(r['total_return'])
        results[label] = {'random': r_random, 'multi': multi_rets}
        r_avg = sum(r_random['rets'])/len(r_random['rets'])
        m_avg = sum(multi_rets)/12
        worst_mdd = min(r_random['mdds'])
        baseline_avg = results.get('baseline (3,10,3) 균등 b=0', {}).get('random', {}).get('seed_avgs', [])
        baseline_multi = results.get('baseline (3,10,3) 균등 b=0', {}).get('multi', [])
        if baseline_avg and 'baseline (3,10,3) 균등 b=0' not in label:
            r_lifts = [b_ - a for a, b_ in zip(baseline_avg, r_random['seed_avgs'])]
            r_wins = sum(1 for l in r_lifts if l > 0)
            avg_l = sum(r_lifts)/len(r_lifts)
            m_lifts = [b_ - a for a, b_ in zip(baseline_multi, multi_rets)]
            m_min = min(m_lifts)
            vs_str = f'{avg_l:+.2f}%p ({r_wins}/{N_SEEDS})'
            m_min_str = f'{m_min:+.2f}%p'
        else:
            vs_str = '★'
            m_min_str = '—'
        print(f'{label:<37} {r_avg:+7.2f}% {m_avg:+7.2f}% {m_min_str:>9} {worst_mdd:+9.2f}% {vs_str:>17} [{time.time()-t1:.0f}s]')
