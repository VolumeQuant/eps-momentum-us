"""70/30 + C1 boost=8 빠른 측정 + 진입 후 첫날 순위 변동 분석"""
import sys
import sqlite3
import random
...
def simulate(dates_all, data, price_full, weights, entry, exit_, c1_boost,
             start_date=None, track_rank_changes=False):
    slots = len(weights)
    if start_date:
    ...
    for di, today in enumerate(dates):
        ...
        # Track rank changes for slot 1 holdings
        if track_rank_changes and di > 0:
            for tk, info in portfolio.items():
                if info.get('slot_idx') == 0 and 'entry_di' in info:
                    days_held = di - info['entry_di']
                    if 1 <= days_held <= 3:
                        cur_rank = new_ranks.get(tk, 999)
                        rank_change_log.append({
                            'tk': tk, 'entry_d': info.get('entry_d'),
                            'days_held': days_held, 'cur_rank': cur_rank,
                        })
        day_ret = 0
        ...
        if len(portfolio) < slots:
            ...
            if new_entries:
                existing = [(tk, today_data.get(tk, {}).get('score_100', 0), 'old', info)
                            for tk, info in portfolio.items()]
                all_in_pf = existing + [(tk, sc, 'new', {'entry_price': price, 'entry_di': di, 'entry_d': today}) for tk, price, sc in new_entries]
                all_in_pf.sort(key=lambda x: -x[1])
                new_portfolio = {}
                for i, (tk, sc, typ, info) in enumerate(all_in_pf):
                    w = weights[i] if i < len(weights) else 0
                    if typ == 'old':
                        new_portfolio[tk] = {'entry_price': info['entry_price'], 'weight': w, 'slot_idx': i,
                                             'entry_di': info.get('entry_di', di), 'entry_d': info.get('entry_d', today)}
                    else:
                        new_portfolio[tk] = {'entry_price': info['entry_price'], 'weight': w, 'slot_idx': i,
                                             'entry_di': info['entry_di'], 'entry_d': info['entry_d']}
                portfolio = new_portfolio
    cum = 1.0; peak = 1.0; max_dd = 0
    for r in daily_returns:
        cum *= (1 + r/100); peak = max(peak, cum)
        dd = (cum-peak)/peak*100; max_dd = min(max_dd, dd)
    return {'total_return': (cum-1)*100, 'max_dd': max_dd, 'rank_change_log': rank_change_log}
...
def main():
    print('=' * 110)
    print('70/30 + C1 boost=8 빠른 측정 + 진입 후 순위 변동 분석')
    print('=' * 110)
    ...
    configs = [
        ('70/30 + C1 b=5', [70, 30], 5),
        ('70/30 + C1 b=8', [70, 30], 8),
        ('70/30 + C1 b=10', [70, 30], 10),
        ('80/20 + C1 b=8', [80, 20], 8),
    ]
    print()
    print(f'{"config":<28} {"R avg":>9} {"R lift":>10} {"R wins":>10} {"M avg":>9} {"M lift":>10} {"M wins":>8} {"M min":>10} {"MDD":>9}')
    print('-' * 110)
    for label, w, b in configs:
        ...
        print(f'  {label:<26} {r_avg:+7.2f}% {r_lift:+8.2f}%p {r_wins:>5}/500 {m_avg:+7.2f}% {m_lift:+8.2f}%p {m_wins:>4}/12 {m_min:+8.2f}%p {worst_mdd:+8.2f}%')

    # 진입 후 1, 2, 3일 순위 변동 분석 (80/20 + b=8 시뮬)
    print()
    print('=' * 110)
    print('1위 슬롯 진입 후 1, 2, 3일 순위 변동 분석 — (2,10,2) 80/20 + C1 boost=8')
    print('=' * 110)
    r_track = simulate(dates, data, price_full, [80, 20], 30, 10, 8, track_rank_changes=True)
    log = r_track['rank_change_log']
    print(f'\n1위 슬롯 진입 종목 추적 (Day 1, 2, 3 순위 기록):')
    print(f'{"ticker":<8} {"entry_date":<12} {"days":>5} {"new_rank":>9}')
    by_days = defaultdict(list)
    for entry in log:
        by_days[entry['days_held']].append(entry)
    for days in [1, 2, 3]:
        events = by_days.get(days, [])
        if events:
            print(f'\n  Day {days} 후 순위:')
            for e in events:
                rank_str = f'{e["cur_rank"]}' if e["cur_rank"] != 999 else '(매도/풀밖)'
                print(f'    {e["tk"]:<8} {e["entry_d"]:<12} {days:>5} {rank_str:>9}')

    # 1, 2, 3일 후 순위 하락 빈도 통계
    print()
    print('  순위 변동 통계:')
    for days in [1, 2, 3]:
        events = by_days.get(days, [])
        if not events: continue
        rank_1 = sum(1 for e in events if e['cur_rank'] == 1)
        rank_2 = sum(1 for e in events if e['cur_rank'] == 2)
        rank_3_5 = sum(1 for e in events if 3 <= e['cur_rank'] <= 5)
        rank_6_10 = sum(1 for e in events if 6 <= e['cur_rank'] <= 10)
        rank_out = sum(1 for e in events if e['cur_rank'] > 10)
        total = len(events)
        print(f'    Day {days}: 1위 유지 {rank_1}/{total}, 2위 {rank_2}, 3-5위 {rank_3_5}, 6-10위 {rank_6_10}, 10위밖 {rank_out}')
