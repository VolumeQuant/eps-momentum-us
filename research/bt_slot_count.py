"""E: Slot count BT — 2 vs 3 vs 4 vs 5 슬롯

현재: 3 슬롯
검증: 슬롯 수 변경 시 알파/MDD trade-off

메모리 v81: (2,10,2) 단독 BT +30.92%p, 500/500. 단 MA20과 함께 가야 MDD 개선.
지금 환경 (v80.10c, MA120 production)에서 슬롯 수 영향 재검증.
"""
import sys
import sqlite3
import random
...
def simulate(dates_all, data, price_full, entry, exit_, slots, start_date=None):
    if start_date:
...
def run(entry, exit_, slots, dates, data, price_full, seed_starts):
    rets, mdds, seed_avgs = [], [], []
    for chosen in seed_starts:
        sr = []
        for sd in chosen:
            r = simulate(dates, data, price_full, entry, exit_, slots, start_date=sd)
...
def main():
    ...
    # (entry, exit, slots) configs
    configs = [
        ('(2, 10, 2)', 2, 10, 2),
        ('(3, 10, 3) baseline', 3, 10, 3),
        ('(4, 10, 4)', 4, 10, 4),
        ('(5, 10, 5)', 5, 10, 5),
        # entry=slots 외 variations
        ('(3, 10, 2)', 3, 10, 2),
        ('(3, 10, 4)', 3, 10, 4),
        ('(3, 10, 5)', 3, 10, 5),
    ]
    print()
    print(f'{"config":<25} {"avg":>9} {"med":>9} {"worst MDD":>10} {"avg MDD":>9} {"sharpe":>7}')
    print('-' * 80)
    results = {}
    for name, entry, exit_, slots in configs:
        res = run(entry, exit_, slots, dates, data, price_full, seed_starts)
        results[name] = res
        ...
        print(f'{marker} {name:<23} {avg:+8.2f}% {med:+8.2f}% {worst_mdd:+9.2f}% {avg_mdd:+8.2f}% {sharpe:+6.2f}')

    print()
    print('=' * 100)
    print('paired vs (3,10,3) baseline')
    print('=' * 100)
    print(f'{"config":<25} {"avg lift":>10} {"worst":>10} {"best":>10} {"wins":>10} {"verdict":>15}')
    print('-' * 95)
    base = results['(3, 10, 3) baseline']['seed_avgs']
    for name, *_ in configs:
        ...
        print(f'  {name:<23} {avg_l:+8.2f}%p {worst:+8.2f}%p {best:+8.2f}%p {wins:>5}/{N_SEEDS}  {verdict}')
