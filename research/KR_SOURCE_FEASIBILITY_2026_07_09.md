...
## 권고 (판정일 안건) — ⚠️ 아래 "최종 권고(수정)"이 이 절을 대체함(B안 프로토타입 후 철회)
1. **단기(완료)**: ntm_90d glitch 가드 배포 — 소스 무관하게 방어.
2. ~~중기 US: B안(자체 point-in-time rev90) 전환~~ → **프로토타입 후 철회**(하단 참조).
3. **중기 KR**: WiseReport(A안)로 커버 확대·안정화 검토 — KR 시스템(eps_momentum_kr.py) 레벨 작업.
...
## B안 프로토타입 실측 결과 (pit_rev90_prototype, US 104일) — ⚠️ 슬램덩크 아님
자체 point-in-time rev90(ntm_current 62거래일전 대비) vs yfinance rolling-column rev90 비교:
- **중앙값 괴리 0.2%p** = 평소엔 두 방식 거의 일치(안심).
- 그러나 **15%p+ 괴리 3.7%**, 일부 극단(ARE +214 vs +480, 저분모 blowup 최대 5만%p+).
- **PIT도 자체 문제 있음**: 62거래일≈90일 근사 오차 + ntm_current 저분모 노이즈. 괴리 케이스에서
  PIT가 "정답"이라 단정 못 함(양쪽 다 glitch-free 아님). 잦은 괴리 종목=RMBS·CVNA·BKNG 등 변동성주.
- **결론 수정**: B안(PIT 전환)은 rolling glitch를 자체 노이즈와 맞바꿀 뿐 명백한 개선 아님.
  → **이미 배포한 median-guard(1일 스파이크만 표적 제거)가 더 나은 실용해**(방식 안 바꾸고
  탐지된 오염만 교정). 구조적 개선은 입력데이터 품질(WiseReport 소스)이지 rev90 산식 변경 아님.

## 최종 권고 (수정)
1. **단기(완료)**: ntm_90d median-guard — 표적 스파이크 제거, 최선의 실용 방어.
2. **중기**: KR 소스를 WiseReport 컨센서스로 보강(입력 품질↑, 73~210 불안정 해소) — KR 시스템 레벨.
3. **폐기**: B안(자체 PIT rev90 전면전환) — 프로토타입상 순이득 불명확, 권고 철회.
4. 수집 헬스가드(KR/통합) 이식은 유효.
