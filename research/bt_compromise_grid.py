"""절충 후보 grid — N=12, N=24 multistart로 알파 + worst case 동시 측정"""
import sys
import sqlite3
import random
import statistics
...
def main():
    print('=' * 110)
    print('절충 후보 grid — N=12 + N=24 multistart, worst case 측정')
    print('=' * 110)
    ...
    base_12 = [simulate(dates, data, price_full, *BASE, start_date=sd)['total_return'] for sd in starts12]
    base_24 = [simulate(dates, data, price_full, *BASE, start_date=sd)['total_return'] for sd in starts24]

    candidates = [
        # 참고 (이미 측정된 것)
        ('A: 70/30 b=0',           [70, 30], 30, 10, 0),
        ('B: 80/20 + C1 b=5',      [80, 20], 30, 10, 5),
        # 절충 후보 — (2,10,2) 75/25 시리즈
        ('75/25 b=0',              [75, 25], 30, 10, 0),
        ('75/25 + C1 b=3',         [75, 25], 30, 10, 3),
        ('75/25 + C1 b=5',         [75, 25], 30, 10, 5),
        # 약한 boost
        ('70/30 + C1 b=3',         [70, 30], 30, 10, 3),
        ('80/20 + C1 b=3',         [80, 20], 30, 10, 3),
        ('80/20 + C1 b=4',         [80, 20], 30, 10, 4),
        # 3슬롯 차등 + C1 boost
        ('(3,10,3) 50/30/20 b=0', [50, 30, 20], 30, 10, 0),
        ('(3,10,3) 50/30/20 + C1 b=3', [50, 30, 20], 30, 10, 3),
        ('(3,10,3) 50/30/20 + C1 b=5', [50, 30, 20], 30, 10, 5),
        ('(3,10,3) 60/25/15 + C1 b=5', [60, 25, 15], 30, 10, 5),
        ('(3,10,3) 70/20/10 + C1 b=5', [70, 20, 10], 30, 10, 5),
        ('(3,10,3) 70/15/15 + C1 b=5', [70, 15, 15], 30, 10, 5),
    ]

    print()
    print(f'{"config":<33} {"R lift":>10} {"R wins":>10} {"M12 lift":>10} {"M12 min":>10} {"M24 lift":>10} {"M24 wins":>11} {"M24 min":>11} {"MDD":>8}')
    print('-' * 120)
    for label, w, entry, exit_, b in candidates:
        cfg = (w, entry, exit_, b)
        r_random = run_random(*cfg, dates, data, price_full, seed_starts)
        m12 = [simulate(dates, data, price_full, *cfg, start_date=sd)['total_return'] for sd in starts12]
        ...
        m12_lift = sum(m12_lifts)/12
        m12_min = min(m12_lifts)
        m24_lifts = [b_ - a for a, b_ in zip(base_24, m24)]
        m24_lift = sum(m24_lifts)/24
        m24_wins = sum(1 for l in m24_lifts if l > 0)
        m24_min = min(m24_lifts)
        worst_mdd = min(r_random['mdds'])
        marker = '★' if 'A:' in label or 'B:' in label else ' '
        print(f'{marker} {label:<31} {r_lift:+8.2f}%p {r_wins:>5}/500 {m12_lift:+8.2f}%p {m12_min:+8.2f}%p {m24_lift:+8.2f}%p {m24_wins:>5}/24 {m24_min:+9.2f}%p {worst_mdd:+7.2f}%')
