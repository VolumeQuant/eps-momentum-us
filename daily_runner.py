...
def create_candidates_message(df, status_map=None, exited_tickers=None, rank_history=None, top_n=30, risk_status=None, weighted_ranks=None):
    ...
    lines.append('━━━━━━━━━━━━━━━━━━━')
    lines.append('💡 <b>읽는 법</b>')
    lines.append('✅ 3일 연속 Top 30 → 매수 후보')
    lines.append('⏳ 2일 연속 → 내일 검증 완료')
    lines.append('🆕 오늘 첫 진입 → 관찰')
    lines.append('목록에 있으면 보유, 없으면 매도 검토.')
    lines.append('')
    lines.append('추세 (90→60→30→7→오늘):')
    lines.append('🔥폭등 ☀️강세 🌤️상승 ☁️보합 🌧️하락')
    lines.append('예) ☁️🌤️☀️🔥 = 가속 · 🔥☀️🌤️☁️ = 둔화')
...
- 어제 시장 마감(지수 등락, 주요 원인)에 집중. "이번 주" 전체 요약은 하지 마.
