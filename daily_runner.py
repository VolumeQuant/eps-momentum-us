...
def create_signal_message(selected, earnings_map, exit_reasons, biz_day, ai_content,
    ...
    lines.append('')
    lines.append('━━━━━━━━━━━━━━━')
    lines.append('🛒 <b>매수</b>: 저평가 신호 1·2위 50/50')
    lines.append('   (자격 통과 종목 1개뿐이면 100% 단독)')
    lines.append('🔴 <b>매도</b>: 다음 중 하나')
    lines.append('   ① 순위 10위 밖 <b>AND</b> 가격 &lt; 12일평균선')
    lines.append('   ② EPS 전망 꺾임 (실적 하락 신호)')
    lines.append('📈 <b>추세 보유</b>: 가격 &gt; 12일평균선이면 순위 밀려도 보유')
    lines.append('   → 오르는 winner 일찍 안 팔기 (예: MU/SNDK/STX)')
    lines.append('⚠️ Top 20의 ⚠️ = 추세 약화 (보유 시 추이 관찰)')
    lines.append('※ 시뮬 누적수익률 (세금·거래수수료 미반영)')
