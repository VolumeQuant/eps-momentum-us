...
- 날짜를 엄격히 지켜: {biz_str} 당일에 일어난 일만 써. 전일(어제) 이벤트를 오늘 일처럼 쓰지 마.
- FOMC·CPI·고용 등 주요 일정: 당일 또는 이전에 결과가 나왔으면 "결과"로, 아직 안 나왔으면 "예정"으로 써.
- 개별 종목 급등락은 당일 변동만 언급해. 전일 변동을 오늘 것처럼 쓰지 마.
