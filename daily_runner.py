...
def _tg_wrap(text, width=32):
    """텔레그램 모바일 폭 맞춤 줄바꿈 — 한글/CJK=2칸 표시폭 기준, 단어 경계에서 자름.

    Gemini에 '짧은 문장'을 시켜도 한글 30자(=60칸) 문장이 나와 모바일서 강제 줄바꿈되던
    문제(2026-07-05 사용자 지적)의 결정적 해법: 코드에서 폭을 보장한다."""
    def _w(t):
        return sum(2 if ord(ch) > 0x2E7F else 1 for ch in t)
    out, cur = [], ''
    for word in text.split():
        cand = (cur + ' ' + word).strip()
        if cur and _w(cand) > width:
            out.append(cur)
            cur = word
        else:
            cur = cand
...
def _vm_ai_briefs(entries, today_str):
    ...
    try:
        ...
        prompt = (
            ...
            '★모든 종목은 지금 상장되어 활발히 거래 중이다. 상장폐지·인수 소멸 서술 금지, '
            '2026년 최신 정보를 검색해 확인하라(예: SNDK=2025년 WD에서 분사 재상장한 샌디스크). '
