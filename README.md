# EPS Momentum System v19.2 - Safety & Trend Fusion (US Stocks)
...
- 최근 3일 모두 Part 2에 있는 종목 = ✅ (포트폴리오 포함)
- 3일 미만 = 🆕 (포트폴리오 제외, 관찰)
- 포트폴리오는 ✅ 종목에서만 선정
...
- 사유: MA60↓, 괴리+, 점수↓, EPS↓ 등
- [1/2] 메시지 내 `⛔ 탈락 종목 — 매도 검토` 블록 형태 (`<b>종목명</b> (ticker) — 사유`)

### Survivors (보유 유지)
- [1/2] 메시지 내 `✅ 보유 유지 가능` 섹션
- 매수 후보 목록에 있는 종목은 보유 유지, 빠지면 ⛔ 알림
...
|--------|------|------|---------------|
| 📖 투자 가이드 | 시스템 개요, 선정 과정, 보유/매도 기준 | 개인봇 | **채널+개인봇** |
| [1/2] 🔍 매수 후보 | 시장 컨텍스트(S&P500/나스닥) + adj_gap 순 후보 + ⛔탈락(블록형) + ✅생존 | 개인봇 | **채널+개인봇** |
| [2/2] 🛡️ AI 브리핑 + 🎯 추천 | AI 점검 + 포트폴리오 추천 (하나로 통합) | 개인봇 | **채널+개인봇** |
| 시스템 로그 | DB 적재 결과, 분포 통계 | 개인봇 | 개인봇 |

발송 순서: 📖 **투자 가이드** → [1/2] 🔍 매수 후보 → [2/2] 🛡️+🎯 AI 브리핑+추천 → 시스템 로그

스토리 라인: 📖이해 → 🔍탐색+탈락+생존 → 🛡️+🎯점검+실행. 처음 보는 고객도 흐름을 이해할 수 있도록 설계.
모든 메시지에 **"읽는 법" 가이드를 헤더**에 배치, parse_mode='HTML' 적용.
...
| 버전 | 날짜 | 변경 |
|------|------|------|
| v19.2 | 2026-02-10 | UI 개선: [1/2][2/2] 2메시지 구조, 시장 컨텍스트, Death List 블록형, Survivors 섹션, MAX_WEIGHT 30%, ⏳ 독스트링 수정 |
