...
| [1/2] 🔍 매수 후보 | 시장지수 + Top 30(✅/⏳/🆕) + 어제 대비 이탈 | 개인봇 | **채널+개인봇** |
| [2/2] 🛡️ AI 점검 + 🎯 최종 추천 | AI 위험 점검 + 포트폴리오 5종목 | 개인봇 | **채널+개인봇** |
| 시스템 로그 | DB 적재 결과, 분포 통계 | 개인봇 | 개인봇 |

발송 순서: 📖 **투자 가이드** → [1/2] 🔍 매수 후보 → [2/2] 🛡️+🎯 AI 점검+추천 → 시스템 로그

스토리 라인: 📖이해 → 🔍탐색 → 🛡️+🎯점검+실행.
...
| 버전 | 날짜 | 변경 |
|------|------|------|
| **v20** | **2026-02-11** | **Simple & Clear**: Death List/Survivors 제거→단순 set 비교, Top 30 경계 통일, ⏳ 표시 전용 복원, 투자 가이드 재작성(월가/이익/5단계 퍼널), cold start 🆕, 매도="목록에 없으면 검토" |
