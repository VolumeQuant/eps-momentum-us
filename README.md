# EPS Revision Momentum Strategy v7.2.0 (US Stocks)

미국 주식 대상 **밸류+가격 100점 체계** 전략 시스템 + **GitHub Actions 자동화**

> 🇺🇸 **핵심 철학**: "좋은 사과를 싸게 - 밸류(맛) + 가격(값) 균형"

## v7.2.0 주요 변경 (2026-02-05)

| 기능 | 설명 |
|------|------|
| **GitHub Actions 자동화** | 매일 KST 08:00 자동 실행, 텔레그램 전송, git push |
| **텔레그램 채널/봇 분리** | 채널(공개) + 봇(개인) 동시 전송, Admin 메시지는 봇만 |
| **날짜 로직 개선** | 인사말=한국날짜, 시장데이터=미국 최근 영업일 |
| **섹터 대분류** | industry(세부) → sector(GICS 대분류)로 변경 |
| **메시지 간소화** | "11-26위 중 주목" 섹션 제거 |

## v7.1.x 주요 기능

| 기능 | 설명 |
|------|------|
| **점수 체계** | 밸류 100점 + 가격 100점, 총점 = 밸류×60% + 가격×40% |
...
"**밸류(Quality)**"와 "**가격(Value)**"을 각각 100점으로 평가 후, 60:40 가중 평균으로 총점 산출.
...
total_score REAL,      -- 총점 = 밸류×0.6 + 가격×0.4
...
2. **가중치 최적화**: Quality/Value 비율 튜닝 (현재 60:40)
