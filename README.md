# EPS Momentum System v20 - Simple & Clear (US Stocks)
...
- 어제 대비 이탈 종목을 [1/2] 메시지 하단에 간단히 표시
...
|--------|------|------|---------------|
| 📖 투자 가이드 | 시스템 개요, 5단계 선정, 보유/매도 기준 | 개인봇 | **채널+개인봇** |
| [1/3] 🔍 매수 후보 | 시장지수 + Top 30(✅/⏳/🆕) + 어제 대비 이탈 | 개인봇 | **채널+개인봇** |
| [2/3] 🛡️ AI 리스크 필터 | 매수 후보 위험 요소 필터링 | 개인봇 | **채널+개인봇** |
| [3/3] 🎯 최종 추천 | 퍼널 요약 + 비중 한눈에 보기 + 포트폴리오 5종목 | 개인봇 | **채널+개인봇** |
| 시스템 로그 | DB 적재 결과, 분포 통계 | 개인봇 | 개인봇 |

발송 순서: 📖 투자 가이드 → [1/3] 🔍 매수 후보 → [2/3] 🛡️ AI 리스크 필터 → [3/3] 🎯 최종 추천 → 시스템 로그
...
비중: abs(adj_gap) 비례, 5% 단위, **종목당 상한 30%** (`MAX_WEIGHT=30`). Gemini가 종목별 선정 이유 생성.
✅ 종목 부족 시 있는 만큼만 추천. 0개면 "관망 권장" 메시지.

### 시장 컨텍스트
[1/2] 메시지 상단에 S&P 500 / 나스닥 전일 종가 및 등락률 표시.
...
SESSION_HANDOFF.md        # 설계 결정 히스토리 (v1~v20)
```
...
| 버전 | 날짜 | 변경 |
|------|------|------|
| **v20** | **2026-02-11** | **Simple & Clear**: Death List 제거→set 비교, Top 30 통일, ⏳ 표시 전용, 투자 가이드 재작성, 메시지 3개 분리([1/3]매수+[2/3]AI 리스크 필터+[3/3]최종 추천), 포트폴리오 퍼널 요약+비중 한눈에 보기, cold start 🆕 |
| v19.2 | 2026-02-10 | UI 개선: [1/2][2/2] 구조, 시장 컨텍스트, Survivors, MAX_WEIGHT 30%, ⏳ 삭제(✅/🆕 2단계), 날짜 biz_day 통일 |
| v19.1 | 2026-02-10 | 🆕 전용 포트폴리오 추천 버그 수정 (✅ 없으면 관망 권장), 전략 근거 문서화 |
