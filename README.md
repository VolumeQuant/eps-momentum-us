# EPS Revision Momentum Strategy v7.2.3 (US Stocks)

미국 주식 대상 **밸류+가격 100점 체계** 전략 시스템 + **GitHub Actions 자동화**

> 🇺🇸 **핵심 철학**: "좋은 사과를 싸게 - 밸류(맛) + 가격(값) 균형"

## v7.2.3 주요 변경 (2026-02-06)

| 기능 | 설명 |
|------|------|
| **장중 데이터 필터링** | 16시(EST) 기준 장마감 여부 판단, 장중엔 전일 마감가만 사용 |

## v7.2.2 주요 변경 (2026-02-05)

| 기능 | 설명 |
|------|------|
| **Option A 적용** | 매수 가능 종목만 표시 (RSI < 70 + 매수 액션), 순위 = 매수 우선순위 |
| **메시지 통합** | 2-3개 메시지 → 1개로 통합 (11위 이후는 별도) |
| **섹터 분석 제거** | 고객에게 불필요한 정보 제거 |
| **선정이유 간소화** | 불릿 포인트 → 한 줄 표시 (매출/영업익/ROE) |
| **리스크 섹션 제거** | 룰베이스로 의미 없는 정보 제거 |

## v7.1.x 주요 기능

| 기능 | 설명 |
|------|------|
| **점수 체계** | 밸류 100점 (EPS 모멘텀), 진입 타이밍은 액션 분류로 판단 |
| **EPS 모멘텀 가중치** | 최근일수록 높은 가중치 (7일 24점 > 30일 22점 > 60일 18점 > 90일 16점) |
| **리스크 자동 생성** | 가격/밸류/섹터별 리스크 문구 자동 생성 |
| **순위 = 매수 우선순위** | 별도 핵심추천 없음, 순위가 곧 우선순위 |

## 버전 히스토리

| 버전 | 날짜 | 주요 변경 |
|------|------|----------|
| **v7.2.3** | 2026-02-06 | 장중 데이터 필터링 - 16시(EST) 기준 장마감 여부 판단, 전일/당일 마감가 구분 |
| v7.2.2 | 2026-02-05 | Option A 적용 (매수 가능만 표시), 메시지 1개 통합, 섹터/리스크 제거, 선정이유 간소화 |
| v7.2.0 | 2026-02-05 | GitHub Actions 자동화, 텔레그램 채널/봇 분리, 섹터 대분류, 날짜 로직 개선 |
| v7.1.8 | 2026-02-05 | 리스크 문구 개선 (밸류/가격 약점 + 구체적 섹터 설명) |
| v7.1.7 | 2026-02-05 | 섹터별 리스크 사전 확대 (12개 GICS 섹터) |
| v7.1.6 | 2026-02-05 | 뉴스 기능 제거 (자동화 안정성 우선) |
| **v7.1** | 2026-02-05 | 밸류+가격 100점 체계, 텔레그램 완전 자동화 |
...
"**밸류(Quality)**"로 종목 순위를 결정하고, "**액션 분류**"로 진입 타이밍을 판단.
...
| 항목 | 배점 | 기준 |
|------|------|------|
| **7일 EPS 변화율** | 24점 | 최근 변화가 가장 중요 |
| **30일 EPS 변화율** | 22점 | - |
| **60일 EPS 변화율** | 18점 | - |
| **90일 EPS 변화율** | 16점 | 장기 변화 |
| **정배열 보너스** | 20점 | C > 7d > 30d > 60d |

```python
def calculate_quality_score_v71(eps_chg_7d, eps_chg_30d, eps_chg_60d, eps_chg_90d, is_aligned):
    score = 0
    # 기간별 가중치 (최근일수록 높음)
    if eps_chg_7d and eps_chg_7d > 0: score += min(24, eps_chg_7d * 2.4)
    if eps_chg_30d and eps_chg_30d > 0: score += min(22, eps_chg_30d * 0.73)
    if eps_chg_60d and eps_chg_60d > 0: score += min(18, eps_chg_60d * 0.3)
    if eps_chg_90d and eps_chg_90d > 0: score += min(16, eps_chg_90d * 0.16)
    # 정배열 보너스
    if is_aligned: score += 20
...
| 항목 | 배점 | 기준 |
|------|------|------|
| **RSI 점수** | 40점 | 30↓:40, 30-40:35, 40-50:25, 50-60:15, 60-70:10, 70+:5 |
| **52주 위치** | 30점 | -25%↓:30, -20%↓:25, -15%↓:20, -10%↓:15, -5%↓:10 |
| **거래량** | 20점 | 평균 대비 증가율 반영 |
| **기본 점수** | 10점 | 통과 종목 기본 |

**신고가 돌파 (v7.1.1+)**: 52주 고점 -2% 이내면 감점만 안 함 (보너스 없음, 비싸면 비싼 것)
...
def calculate_value_score_v71(rsi, from_52w_high, volume_ratio):
    ...
    # RSI (40점)
    if rsi <= 30: score += 40
    elif rsi <= 40: score += 35
    elif rsi <= 50: score += 25
    # ...
    # 52주 위치 (30점)
    if from_52w_high <= -25: score += 30
    elif from_52w_high <= -20: score += 25
    # ...
    # 신고가 돌파 모멘텀
    if from_52w_high >= -2: score = max(score, 80)
    return min(100, score)
```
...
**리스크 자동 생성 (v7.1.8)**:
- 가격: RSI 수치, 당일 등락률, 고점 근접, 가격점수
- 밸류: 밸류점수 낮으면 "EPS 모멘텀 약함"
- 섹터: 구체적 위험 요인 (12개 GICS 섹터)

**자동 생성 함수**:
- `generate_rationale_bullets_v71(row)` - 선정이유 불릿 포인트 2-3개
- `generate_risk_v71(row)` - 리스크 자동 생성
- 순위 = 매수 우선순위 (별도 핵심추천 분류 없음)

### v7.1 TOP 10 결과 예시 (2026-02-05)
...
```sql
quality_score REAL,    -- 밸류 Score (v7.1)
value_score REAL,      -- 가격 Score (v7.1)
total_score REAL,      -- 총점 = 밸류 (EPS 모멘텀)
eps_chg_7d REAL,       -- 7일 EPS 변화율
eps_chg_30d REAL,      -- 30일 EPS 변화율
eps_chg_90d REAL,      -- 90일 EPS 변화율
...
## GitHub Actions 자동화 (v7.2.0)

### 스케줄
- **실행 시간**: UTC 23:00 = KST 08:00 (다음날)
- **실행 요일**: 월~금 (미국 주말 제외)
...
| `TELEGRAM_CHAT_ID` | 채널 ID (공개 메시지용) |
| `TELEGRAM_PRIVATE_ID` | 개인 챗 ID (Admin 메시지용) |

### 텔레그램 전송 분리
| 환경 | User 메시지 | Admin 메시지 |
|------|-------------|--------------|
| 로컬 | 봇만 | 봇만 |
...
eps-momentum-us/
├── .github/workflows/
│   └── daily-screening.yml  # GitHub Actions 워크플로우
├── daily_runner.py          # 자동화 시스템 (스크리닝 + 텔레그램 + Git)
├── eps_momentum_system.py   # 코어 로직 (스코어링, 필터)
├── sector_analysis.py       # 섹터 분석 + ETF 추천
├── requirements.txt         # Python 의존성
├── config.json              # 로컬 설정 (텔레그램, Git 등)
├── eps_momentum_data.db     # SQLite DB (백테스트용, 매일 누적)
