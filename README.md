...
4. **✅ 검증 종목만 카운트** (v79): ⏳/🆕는 스킵하되 verified_count에 포함하지 않음 → ✅ 기준 3종목 채움
5. **최대 3슬롯** 보유
...
v79부터 Signal 매수 후보는 ✅만 카운트. ⏳/🆕가 w_gap 순위 상위에 있으면 스킵 후 다음 ✅ 종목 선정.
...
### GitHub Actions (자동)
- **일일 스케줄**: 월~금 **KST 20:00** (cron: `'0 11 * * 1-5'` UTC) — v78에서 변경 (마감 직후 → 다음날 장 시작 전, EPS 데이터 안정화)
