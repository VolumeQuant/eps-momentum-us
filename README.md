...
Forward 12개월 EPS(NTM EPS) 기반 모멘텀 시스템. "파괴적 혁신 기업을 싸게 살래" 철학으로, EPS 괴리율(adj_gap) 절대값을 기반으로 저평가 종목을 선별. adj_gap < -7(매력도 85점+)이면 진입, adj_gap > +1(매력도 45점-)이면 매도 검토. MA120 + 3일 연속 검증으로 신뢰도를 높이고, AI(Gemini)가 위험 신호를 점검한 뒤 최종 포트폴리오를 추천.
...
### 매수 후보 선정 (adj_gap 절대값 기반)

**철학**: adj_gap(EPS 괴리율) 절대값이 유일한 순위 신호. 음수가 클수록 저평가. 혁신 없는 종목은 하드필터(매출 ≥10%)로 제외.
...
- **검증 기준: `adj_gap < -7`** — 각 날의 adj_gap이 진입 기준(-7) 미만인지 확인
- 최근 3일 모두 adj_gap < -7 = ✅ (포트폴리오 대상)
- 최근 2일 연속 adj_gap < -7 = ⏳ (표시만, 내일 검증 가능)
- 그 외 = 🆕 (오늘 첫 진입)
- 포트폴리오는 ✅ 종목에서만 선정

### 매도 기준: adj_gap > +1 (매력도 45점 이하)
- **절대값 매도 기준**: adj_gap > +1이면 매도 검토 (EPS 대비 주가 과대 반영)
- 어제 대비 이탈 종목 + 이탈 사유([괴리율↑], [MA120↓], [EPS↓], [매출↓], [저커버리지], [순위밀림]) 표시
- `classify_exit_reasons()`: adj_gap > +1 → "괴리율↑" 우선, 그 외 기존 사유 유지
...
- **유일한 순위 신호 (v52)**: adj_gap < -7 = 진입, adj_gap > +1 = 매도
