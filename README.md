# EPS Momentum System v19.1 - Safety & Trend Fusion (US Stocks)
...
| [1/3] 🔍 매수 후보 | adj_gap 순, ✅검증/🆕신규/📉탈락 | 개인봇 | **채널+개인봇** |
| [2/3] 🛡️ AI 점검 | 시장 동향 + 위험 신호 분석 + 어닝 | 개인봇 | **채널+개인봇** |
| [3/3] 🎯 최종 추천 | ✅ 종목 중 리스크 필터 통과 Top 5 + 활용법 | 개인봇 | **채널+개인봇** |
| 시스템 로그 | DB 적재 결과, 분포 통계 | 개인봇 | 개인봇 |

발송 순서: 📖 **투자 가이드** → [1/3] 🔍 매수 후보 → [2/3] 🛡️ AI 점검 → [3/3] 🎯 **최종 추천** → 시스템 로그

스토리 라인: 📖이해 → 🔍탐색 → 🛡️점검 → 🎯실행. 처음 보는 고객도 흐름을 이해할 수 있도록 설계.
모든 메시지에 **"읽는 법" 가이드를 헤더**에 배치, [3/3]에 보유 기간·매도 기준·활용법 포함.
