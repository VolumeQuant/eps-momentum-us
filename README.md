...
Forward 12개월 EPS(NTM EPS) 기반 모멘텀 시스템. 이익 추정치 변화율로 종목을 선별하고, 주가 미반영 종목을 매수 후보로 제시. AI(Gemini)가 매수 후보 데이터를 분석하여 투자 브리핑 제공.
...
- **Part 2 (매수 후보)**: adj_gap 순 (방향 보정 괴리율) — EPS 개선이 아직 주가에 반영 안 된 종목
  - 필터: adj_score > 9, EPS 변화율 > 0, Fwd PE > 0
  - 정렬: adj_gap = fwd_pe_chg × (1 + clamp(direction/30, -0.3, +0.3))
  - fwd_pe_chg = 가중평균 Fwd PE 변화 (7d×40% + 30d×30% + 60d×20% + 90d×10%)
  - 방향 보정: 가속 EPS → 저평가 강화, 감속 EPS → 저평가 약화
  - 표시 4줄: 순위+종목명 / 업종+날씨+패턴 / EPS%·주가%·괴리 / 의견 ↑N ↓N
...
- Part 2 정렬은 adj_gap (방향 보정 괴리율) 별도 사용
- 고객 표시명: "EPS 점수"
...
**방향 보정 (adj_gap)**:
adj_score와 동일한 direction 보정을 괴리율에도 적용.
```
...
### 추세 표시: 5단계 날씨 아이콘 + 12패턴
4개 세그먼트의 방향과 크기를 날씨 아이콘으로 시각화 (순서: 과거→현재):
- ☀️ 맑음 (>10%) — 강한 상승
- 🌤️ 상승 (2~10%)
- ☁️ 흐림 (-2~2%) — 보합/미약한 변동
- 🌧️ 하락 (-10~-2%)
...
각 아이콘의 형태가 완전히 달라 모바일에서도 즉시 구분 가능.
12개 기본 패턴(횡보/하락/전구간 상승/꾸준한 상승/상향 가속/최근 급상향/중반 강세/상승 등락/상향 둔화/반등/추세 전환/등락 반복)에 ☀️⛈️ 강도 수식어(폭발적~/급~/중반 급등/폭발적 등락/급등 후 둔화 등) 조합.
...
**제거한 필터** (시스템 철학과 모순):
- ~~급락 (price_chg < -20)~~: 주가 하락이 adj_gap을 만드는 원인 → 기회를 위험으로 오분류
- ~~괴리 (|price/eps| > 5)~~: adj_gap이 측정하는 것과 동일 → 1등을 제거하는 구조
- ~~감속 (direction < -10)~~: adj_score가 이미 direction 반영 → 이중 처벌
- ~~하향 (rev_down ≥ 1, ≥ up)~~: 하향 1건으로 시스템 전체 신호 무시 → 과도
...
- 📊 매수 후보 분석: 섹터 집중도, 주목 종목, ⚠️ 종목, EPS 모멘텀 방향성
- 📅 어닝 주의: yfinance `stock.calendar`에서 2주내 실적발표 직접 조회 (AI 비의존)
- 데이터에 있는 정보만 사용, 없는 내용 생성 금지, temperature 0.3
- 출력: 2000자 이내
...
|--------|------|------|---------------|
| Part 1 EPS 모멘텀 랭킹 | Top 30, EPS 점수 순 | 개인봇 | **채널+개인봇** |
| Part 2 매수 후보 (핵심) | Top 30, adj_gap 순 | 개인봇 | **채널+개인봇** |
| AI 브리핑 | 시장 동향 + 데이터 분석 + 어닝 | 개인봇 | **채널+개인봇** |
...
| v9.2 | 2026-02-08 | AI 브리핑 전환: 뉴스 스캐너→데이터 분석, 어닝 yfinance 직접 조회, 청크 분할 버그 수정 |
| v9.1 | 2026-02-08 | 트래픽 라이트 12패턴 확장 (피크 위치 기반 분류), "상향 가속" 과다 문제 해결 |
