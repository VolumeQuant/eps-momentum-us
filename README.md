# EPS Momentum System v70 (US Stocks)

Forward 12개월 EPS(NTM EPS) 기반 모멘텀 시스템. **"파괴적 혁신 기업을 싸게 살래"** 철학으로, w_gap(가중 괴리율)을 기반으로 저평가 종목을 선별한다. 괴리율이 음수일수록 EPS 개선 대비 주가가 덜 반영된 상태(= 저평가). MA120 + 리스크 필터로 신뢰도를 높이고, AI(Gemini 2.5 Flash)가 위험 신호를 점검한 뒤 최대 3종목을 매수 후보로 제시한다. 최대 5종목 보유.
...
### 진입 조건
`select_display_top5()` — 매일 최대 3종목 매수 후보 선정, 누적 최대 5종목 보유:
...
5. 여러 날에 걸쳐 **최대 5슬롯** 누적 보유

> v70 변경: 슬롯 3→5 확장. 진입은 여전히 Top 3만. shrinkage 논리 — 28거래일 백테스트는 과적합 위험이 있어 집중(3)→분산(5)으로 보수적 선택.
...
### 포지션 사이징: 역변동성 (Inverse Volatility)
종목별 비중을 최근 5일 변동성의 역수로 결정:
```
weight(i) = (1 / vol_i) / Σ(1 / vol_j)
```
- 변동성 큰 종목 → 적게, 작은 종목 → 많이
- 백테스트: CAGR +226%→+299%, MDD -13.9%→-10.1% (수익↑ + 위험↓)
- 6개 개선안 테스트 중 유일하게 채택
...
📈 시스템 누적 수익률 +11.1% (25거래일)
    같은 기간 S&P500은 -5.7%
```
- `_get_system_performance()`: DB 기반 복리 재투자 백테스트 리플레이 (역변동성 비중 적용)
